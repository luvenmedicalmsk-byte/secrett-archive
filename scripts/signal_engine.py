...
def _cluster(events):
    ...
    for i in range(n):
        for j in range(i+1,n):
            ...
            # вулкан/шторм) в одном месте — РАЗНЫЕ процессы. Иначе «Наводнение» вбирает
            # пожары и маловодье. None (общее метеоявление) не блокирует.
            if PHEN[i] and PHEN[j] and PHEN[i]!=PHEN[j]: continue
