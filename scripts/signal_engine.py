...
def _enrich_macro(macro, members, now):
    ...
    if _pl:
        ...
        _wz=_MANUAL_WATCH.get(macro.get('signal_id'))
        if _wz:
            _stx['watch_zones']={'covered':[dict(z) for z in _wz.get('covered',[])],
                                 'open':[dict(z) for z in _wz.get('open',[])]}
...
_MANUAL_PLACES = {
 'geop-macroинфра-ru-e487': [
   ...
   'Ленинградская область', 'Республика Крым',
   'Свердловская область', 'Удмуртская Республика',
 ],
}

# Приоритетные зоны наблюдения (сценарий A): где новые подтверждения
# максимально изменят гипотезу. covered — уже затронуто, open — под наблюдением.
_MANUAL_WATCH = {
 'geop-macroинфра-ru-e487': {
   'covered': [
...
_MANUAL_WAVES = {
 'geop-macroинфра-ru-e487': [
   ...
   ('2026-07-18', 'Логистический центр, Электросталь (Московская область)', 70),
   ('2026-07-22', 'Склад, Краснодар', 68),
   ('2026-07-22', 'Склад, Невинномысск (Ставропольский край)', 68),
   ('2026-07-24', 'Объект, Шушары (Ленинградская область)', 72),
   ('2026-07-24', 'Объект, Уткина Заводь (Ленинградская область)', 72),
   ('2026-07-24', 'Объект, Новосаратовка (Ленинградская область)', 72),
   ('2026-07-24', 'Объект (Республика Крым)', 72),
   ('2026-07-25', 'Логистический центр Wildberries, Екатеринбург — работа хаба остановлена', 78),
