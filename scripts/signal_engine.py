...
_PROC_TYPE=[(r'продаж|ритейл|розничн|магазин|дивиденд','Розничная торговля'),
 ...
 (r'контрабанд\w* (?:товар|груз|партии)','Криминальный оборот'),(r'дрон.{0,15}завод|производств дрон','Оборонное производство')]
# ═══ ECONOMY CANON COMPANION (batch 1/2/3) — «финансовая устойчивость в процессах» ═══
# Реестры РАЗНЫЕ: _CANON_TYPE (fetch_events, канонизация) знал batch-типы, а _PROC_TYPE
# (Process Engine, legacy typing) — нет. События без canon_type падали в «Экономический
# сигнал», т.е. специализация не доходила до процессов. Зеркалируем правила канона.
# Вставка В НАЧАЛО: scoring `if sc>bs` строгий → при равном score побеждает первый в
# таблице; специфичные финансовые правила должны выигрывать у общего 'продаж'→Retail.
# За флагом ECONOMY_PROC_TYPES=False → байт-идентично.
ECONOMY_PROC_TYPES = True
_ECON_PROC_INSERT = [
 ...
 (r'дефолт|госдолг|гос\w*\s+долг|суверенн\w*\s+долг|долгов\w*\s+кризис|гособлигац|казначейск\w*\s+(?:облигац|бумаг)|потолок\s+долг|реструктуризац\w*\s+долг','Государственный долг'),
 (r'рецесс|экономическ\w*\s+спад|спад\s+экономик|сокращени\w*\s+ввп|падени\w*\s+ввп|стагнац','Экономический спад'),
 (r'фондов|финансов\w*\s+рынок|мосбирж|биржев|\bбиржа\b|котировк|облигац|\bбонд\b|\betf\b|\bcds\b|ключев\w*\s+ставк|учётн\w*\s+ставк|ставк\w*\s+(?:цб|фрс|ецб)|процентн\w*\s+ставк','Финансовый рынок'),
 (r'торгов\w*\s+(?:профицит|дефицит|баланс|оборот|сальдо)|внешнеторгов\w*\s+оборот','Торговый баланс'),
 (r'бюджетн\w*\s+(?:дефицит|профицит|расход|доход|правил)|дефицит\s+бюджет|профицит\s+бюджет|фискальн\w*\s+политик|госрасход|секвестр','Государственные финансы'),
 (r'безработиц|рынок\s+труда|занятост|массов\w*\s+увольнен|дефицит\s+(?:кадр|персонал)|заработн\w*\s+плат','Рынок труда'),
 (r'редкоземельн\w*\s+(?:металл|элемент)|критическ\w*\s+(?:сырь|материал|минерал)|дефицит\s+(?:лити|коба|никел|урана|удобрен|полупроводник|чипов)','Стратегические ресурсы'),
 (r'пузыр\w*\s+(?:на\s+рынке|актив|недвижимост|крипт)|перегрет\w*\s+рынок|перегрев\s+рынка|ипотечн\w*\s+кризис|переоценённ\w*\s+актив|переоценк\w*\s+(?:актив|дол\w*|акци|портфел|стоимост)|бумажн\w*\s+(?:прибыл|доход)|доход\w*\s+(?:лишь\s+|только\s+)?на\s+бумаге','Пузырь активов'),
]
if ECONOMY_PROC_TYPES:
    _PROC_TYPE[0:0] = _ECON_PROC_INSERT
_TYPE_DOMAIN={
...
_TYPE_TEMPO={
 ...
 'Эпидемиологический риск':'medium','Миграционная политика':'medium','Криминальный оборот':'medium','Социальный процесс':'medium',
...
# Якорь географии макро-процесса: страны, которые ДОЛЖНЫ присутствовать
# в свидетельстве, чтобы оно попало в хронику этого места. Используется
# только для фильтра хроники; на состав членов, охват и давление
# не влияет. Место без записи здесь фильтр не применяет.
# Словесные маркеры якоря: резолвер видит топонимы, но не субъекта
# действия. Используются как вторая проверка гео-фильтра хроники.
_MACRO_ANCHOR_RX = {
    'Россия — Украина': _re.compile(r'росси|рф\b|москв|кремл|всу\b|украин|киев|зеленск', _re.I),
    'Россия':           _re.compile(r'росси|рф\b|москв|кремл|российск', _re.I),
    'Украина':          _re.compile(r'украин|киев|всу\b|зеленск', _re.I),
    'Ближний Восток':   _re.compile(r'иран|израил|сири|ирак|йемен|саудов|ливан|газ[аеы]\b|катар|оаэ|'
                                    r'ормуз|персидск|хусит|ближн\w+ восток|турци', _re.I),
    'Европа':           _re.compile(r'\bе[сu]\b|евросоюз|европейск|европ|брюссел|герман|франц|итали|'
                                    r'испан|польш|греци|нидерланд|бельги|швеци|австри', _re.I),
    'Китай':            _re.compile(r'кита|пекин|шанха', _re.I),
    'Северная Америка': _re.compile(r'сша|америк|вашингтон|канад|мексик', _re.I),
}

_MACRO_ANCHOR = {
    'Россия — Украина': {'RU', 'UA'},
    'Россия':           {'RU'},
    'Украина':          {'UA'},
    'Ближний Восток':   {'IR','IL','SY','IQ','YE','SA','LB','QA','AE','KW','OM','BH','JO','PS','TR','EG'},
    'Европа':           {'AT','BE','CH','CZ','DE','ES','EU','FR','GB','GR','HU','IT','NL','PL','PT',
                         'RO','SE','SK','DK','FI','NO','IE','BG','HR','RS','SI','LT','LV','EE','LU',
                         'MT','CY','IS','AL','MK','ME','BA','MD','UA'},
    'Китай':            {'CN'},
...
def _enrich_macro(macro, members, now):
    ...
    # ── ГЕО-ФИЛЬТР ХРОНИКИ (аудит 14.08) ────────────────────────────────
    # Под-процесс попадает в макро по паре (тип, место). Но его evidence
    # могут относиться к другому театру: событие «США предотвратили
    # иранскую ракетную атаку на Украину» даёт place='Украина', и весь
    # под-процесс с иранскими свидетельствами уходит в российско-
    # украинский кластер. Аудит 35 процессов нашёл 45 таких записей
    # из 428 — «В Иракском Курдистане серия атак», «Хуситы атаковали
    # саудовское судно», «КНДР запустила баллистическую ракету».
    #
    # Якорь макро — страны, названные в его собственном месте
    # (_MACRO_ANCHOR), а не объединение стран всех членов: последнее
    # включает союзников и упоминания и фильтром быть не может.
    # Свидетельство проходит, если хотя бы одна его страна в якоре
    # или география не распознана вовсе (не отбрасываем при незнании).
    # Фильтр применяется ТОЛЬКО к процессам с однозначной географией:
    # стихия происходит в конкретном месте, и «Гроза: Ингушетия»
    # в европейском процессе — явная ошибка. Геополитика, санкции
    # и военные конфликты исключены: там субъект действия часто
    # не назван в тексте («Пакет санкций против России перенесён»
    # вводит ЕС, «ВСУ атаковали волгоградский НПЗ» — Украина),
    # и гео-фильтр отбросил бы такие события как чужие.
    # 'Военные удары' добавлены после проверки баланса: в российско-
    # украинском процессе фильтр снимет 21 чужую запись («Хуситы
    # заявили о нападении на саудовский НПЗ», «Пентагон: 153 убитых
    # в Йемене», «В Иракском Курдистане серия атак», «КНДР запустила
    # баллистическую ракету») и потеряет одну обоснованную — «ВСУ
    # атаковали волгоградский НПЗ», где резолвер видит только RU.
    # Двадцать к одному, тогда как для санкций баланс был обратным,
    # поэтому они остаются вне фильтра.
    _GEO_STRICT_TYPES = ('Шторм', 'Тепловая волна', 'Наводнение',
                         'Водный дефицит', 'Пожарная активность',
                         'Климатический сигнал', 'Военные удары')
    _ptype_now = str(macro.get('process_type') or '')
    _anchor = (_MACRO_ANCHOR.get(str(macro.get('process_place') or ''))
               if _ptype_now in _GEO_STRICT_TYPES else None)
    _anchor_rx = _MACRO_ANCHOR_RX.get(str(macro.get('process_place') or ''))
    def _tl_geo_ok(ttl):
        if not _anchor:
            return True
        _low = (ttl or '').lower()
        ...
        if _pl & _anchor:
            return True
        # СМЯГЧЕНИЕ. Резолвер возвращает страны-топонимы, но не субъекта
        # действия: «Пакет санкций против России перенесён» даёт только RU,
        # хотя санкции вводит ЕС. «ВСУ атаковали волгоградский НПЗ» даёт RU
        # при украинском акторе. Такие события относятся к якорю по смыслу
        # и отбрасываться не должны.
        # Поэтому вторая проверка: упоминается ли якорь словами — названием
        # союза, страны, аббревиатурой армии. Только при отсутствии и того
        # и другого запись считается чужой.
        if _anchor_rx and _anchor_rx.search(_low):
            return True
        return False
    _tl=[]; _seen=set(); _tl_dropped=0
    for m in _nsrc:
        for e in (m.get('evidence') or []):
            ttl=(e.get('title') or '').strip()
            if not ttl or _is_noise(ttl): continue
            if not _tl_geo_ok(ttl):
                _tl_dropped += 1
                continue
            k=ttl[:50]
...
def _reconstruct_macro(signals, now):
    ...
    _CONFLICT_CLUSTERS=[
        # 'Глобально' убрано: place=Глобально означает, что событие
        # не привязано к театру действий. Условие pair<=countries требует
        # лишь ПРИСУТСТВИЯ RU и UA среди стран, а не конфликта между ними,
        # поэтому в кластер попадали чужие события: атаки хуситов на
        # саудовский НПЗ, удары США по иракским объектам, инциденты
        # в Красном море. У них RU и UA оказывались в списке стран
        # из-за упоминаний в тексте, а place был 'Глобально'.
        ('Россия — Украина',{'RU','UA'},{'Россия','Украина','Европа','Крым','Азовское море','Балтийское море'}),
...
def _build_one_signal(evs, meta=None):
    ...
    for x in sorted(evs,key=lambda x:-x.get('severity',0)):
        ...
        if re.search(r'бодяж|фуфло|туфта|брехн|пал[её]ва|галим|развалюх|обосра|зашкварн', _ttl.lower()):
            continue
        # SPEC-013 Phase 1.1 (Shadow Coverage): sic_class сохраняется В EVIDENCE.
        # Раньше Admission Shadow сопоставлял процессы с событиями по title[:60] и покрывал
        # лишь 27% (132 из 492): процесс живёт неделями, а его события давно вышли из окна
        # ленты — сопоставить их не с чем. Класс, зафиксированный в момент построения,
        # остаётся с процессом навсегда. ОБОГАЩЕНИЕ ДАННЫХ: поведение Process Builder не
        # меняется, sic_class ни на что не влияет (Admission не реализован).
    ...
    # Стабильность при высокой тяжести угрозы не снижает. Прежде множитель
    # +0.15 получал только растущий тренд, и процесс с тяжестью 86,
    # давлением 95 и 254 свидетельствами («Военный конфликт — Россия —
    # Украина») имел приоритет 59, тогда как растущий с тяжестью 84,
    # давлением 91 и 47 свидетельствами — 85. Стабильно высокий уровень
    # опаснее растущего низкого, а не наоборот.
    #
    # Порог по sev, а не по pressure: давление считается позже приоритета
    # и здесь недоступно. Sev>=80 отбирает те же процессы — на корпусе
    # из 35 системных изменится один.
    # Спад множитель не получает: там показатель снижается.
    _hold = 1 if (sev >= 80 and not _rising(trend)
...
def write_signals_json(events, path):
    ...
    if _pex is not None:
        try:
            ...
            pass
    # ADR-018: Relationship Engine — связи между процессами (PREL-1: не меняет процессы)
