...
NOISE_PATTERNS = [
    ...
    # Годовщина: ретроспектива к памятной дате, а не событие сегодняшнего дня.
    # Ищется ТОЛЬКО В ЗАГОЛОВКЕ: в summary «7300 лет назад» это геология,
    # и проверка на срезе показала три ложных срабатывания на климате.
    # Диапазон лет ограничен тремя цифрами по той же причине.
    (re.compile(r"(?:спустя|прошло)\s+([1-9]\d{0,2})\s+(?:год|лет|года)|"
                r"\bгодовщин\w+|\bв\s+память\s+о\b|\bпамятн\w+\s+дат", re.I),
     "годовщина (ретроспектива)", 0.75),
    # Индивидуальное ЧП: пострадал конкретный человек. Обязательно требуется
    # одушевлённый субъект — без него «обломки БПЛА упали» и «госпитализирован
    # 21 человек» после атаки уходили в шум, хотя это события системного уровня.
    (re.compile(r"\b\d{1,2}-летн(?:ий|яя|его|ей)\s+\w*\s*"
                r"(?:мужчин|женщин|девушк|парен|подрост|ребен|ребён|школьн|студент)|"
                r"\b(?:девушка|женщина|мужчина|подросток|ребенок|ребёнок|школьник|студент\w*)"
                r"\s+\w*\s*(?:выпал|упал|сорвал|провалил)\w*", re.I),
     "индивидуальное ЧП", 0.4),
]
