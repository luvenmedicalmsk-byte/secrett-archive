...
# ═══ РЕЕСТР СОСТОЯНИЙ: АКТИВНЫЙ ВООРУЖЁННЫЙ КОНФЛИКТ (04.09.2026) ═══
# Страновой индекс считается по потоку событий за окно и отвечает на вопрос
# «сколько тяжёлых сигналов пришло», а не «в каком положении страна». Для
# страны в состоянии войны это даёт заниженную оценку: у России 59 при
# драйверах-маловодьях, потому что война даёт мало событий в сутки, а
# усредняется с климатической рутиной.
#
# Проверялась альтернатива — добавить в индекс вес процессов вместо событий.
# Отвергнута измерением: при широкой привязке процессов Канада получала 77,6,
# при строгой Россия давала 69,6 и до 70 не дотягивала ни при каком смешении,
# а Непал с тремя живыми процессами выходил на 95,6. Меняется знаменатель,
# а не суть.
#
# Поэтому введён явный реестр состояний. Это не список «опасных стран»:
# каждая строка имеет дату начала и основание, снимается так же явно.
CONFLICT_REGISTRY = {
    "RUS": {"since": "2022-02-24", "floor": 70,
            "basis": "военный конфликт Россия — Украина, 325 свидетельств в процессе; регулярные удары по территории"},
    "UKR": {"since": "2022-02-24", "floor": 70,
            "basis": "военный конфликт Россия — Украина; взвешенная тяжесть процессов 84,3"},
    "ISR": {"since": "2023-10-07", "floor": 70,
            "basis": "вооружённый конфликт в Газе и региональные удары"},
    "PSE": {"since": "2023-10-07", "floor": 70,
            "basis": "вооружённый конфликт в Газе, удары с потерями среди гражданских"},
    "IRN": {"since": "2026-06-01", "floor": 70,
            "basis": "удары США по территории, атаки на объекты (остров Ларак, Серик)"},
    "SDN": {"since": "2023-04-15", "floor": 70,
            "basis": "гражданская война, перемещение населения внутри страны"},
    "MMR": {"since": "2021-02-01", "floor": 70,
            "basis": "внутренний вооружённый конфликт после смены власти"},
    "YEM": {"since": "2014-09-01", "floor": 70,
            "basis": "гражданская война, удары по морским целям в регионе"},
    "SYR": {"since": "2011-03-15", "floor": 70,
            "basis": "продолжающийся вооружённый конфликт"},
    "MLI": {"since": "2012-01-01", "floor": 70, "basis": "вооружённый конфликт в Сахеле"},
    "BFA": {"since": "2015-01-01", "floor": 70, "basis": "вооружённый конфликт в Сахеле"},
    "NER": {"since": "2023-07-26", "floor": 70,
            "basis": "вооружённый конфликт и попытки смены власти"},
    "SOM": {"since": "2009-01-01", "floor": 70, "basis": "продолжающийся вооружённый конфликт"},
    "COD": {"since": "2022-03-01", "floor": 70, "basis": "вооружённый конфликт на востоке страны"},
    "HTI": {"since": "2024-02-29", "floor": 70,
            "basis": "вооружённое насилие с утратой контроля над территорией"},
}


def conflict_floor(iso3: str):
    """Нижний порог странового индекса и его основание, либо None."""
    rec = CONFLICT_REGISTRY.get((iso3 or "").upper())
...
def build_country_profile(
    ...
    # Порог состояния: поток событий не может опустить индекс страны в
    # активном вооружённом конфликте ниже установленного уровня. Порог только
    # поднимает: если поток даёт больше, берётся значение потока.
    _cf = conflict_floor(iso3)
    _floor_applied = None
    if _cf and composite < _cf["floor"]:
        _floor_applied = {"floor": _cf["floor"], "raw_score": composite,
                          "since": _cf["since"], "basis": _cf["basis"]}
        composite = _cf["floor"]
