...
# Прилагательные с заглавной буквы: не топонимы. Проверяются по суффиксу, а не
# перечнем — перечень пришлось бы пополнять при каждом новом сочетании.
_MERGE_ADJ = _re.compile(r'(ские|ский|ская|ское|ского|ской|ском|скую|ских|скими)$', _re.I)
...
_MANUAL_ALIAS = {
    ...
    'obs-b1f7b236': 'infra-b1f7b236',
    'det-b1f7b236': 'infra-b1f7b236',
}
        ...
        def _stems(text, place='', n=4):
            ...
            for _w in _re.findall(r'[А-ЯЁA-Z][а-яёa-z]{4,}', str(text or '')):
                ...
                if _s in _pw or _w.lower() in _MERGE_STOP or _w.lower() in _MERGE_OPER \
                    ...
                    continue
                # Прилагательные с заглавной («Украинские», «Российские») топонимами
                # не являются. Без этого правила «Украинские беспилотники ударили по
                # складу под Самарой» и «горит центр Wildberries в Новосемейкино
                # Самарской области» — одно и то же событие — расходились по паре
                # «укра» против «ново» и попадали в хронику дважды.
                if _MERGE_ADJ.search(_w):
                    continue
                _out.add(_s)
            return _out
...
def build_infra(events, docs_dir=None):
    ...
    for e in _src:
        ...
        grp,causal=d
        # ═══ СЛИЯНИЕ attack + incident В ОДИН ПРОЦЕСС ═════════════════════════
        # Разделение шло по причинному классу: удар и происшествие давали два
        # процесса одного типа. На карточке это выглядело как «24 подтв.» при
        # 22 у основного — второй процесс содержал «Взрывы возле склада
        # Wildberries в Воронеже» и «Пожар на складе в Ленинградской области».
        #
        # По существу это те же удары по логистике: слово «атака» просто не
        # попало в заголовок, хотя причина та же. Разделять их — значит делить
        # одно явление по формулировке источника.
        #
        # outage (сбой сервиса) и regulation (изменение правил) остаются
        # отдельными: у них другая природа, а не другая формулировка.
        _causal_key = 'damage' if causal in ('attack', 'incident') else causal
        key=hashlib.md5(f"{grp}|{_causal_key}".encode()).hexdigest()[:8]
    ...
    for key,g in groups.items():
        ...
        if FEATURES_LAYER:
            ...
            _OP_HIT = _re.compile(
                ...
                # Глагольные формы возгорания: «пожар» и «возгорание» есть,
                # а «загорелся» не было. «Склад Ozon загорелся после ночной
                # атаки беспилотников» считалось упоминанием без поражения.
                r'загорел\w+|горит|сгорел\w*|вспыхнул\w*|'
                # ОБЪЕКТ ПОСТРАДАЛ ПОСЛЕ АТАКИ: обратный порядок слов.
                # Прежние конструкции требовали «атаковали склад», где объект
                # стоит ПОСЛЕ действия. В «Склад загорелся после атаки»
                # объект стоит первым.
                r'(?:склад|центр|комплекс|объект|хаб)\w*[^.]{0,60}?'
                r'(?:после|в\s+результате)\s+[^.]{0,30}?'
                r'(?:атак|удар|обстрел|нал[её]т)|'
                # ОБЪЕКТ ПЕРЕД ГЛАГОЛОМ и ПАССИВ. «Склад Ozon впервые
                # атаковали беспилотники», «Логистический центр был атакован
                # БПЛА» - прежние конструкции требовали объект СРАЗУ после
                # глагола или связку «после атаки», и оба случая пропускали.
                r'(?:склад|центр|комплекс|объект|хаб|терминал)\w*[^.]{0,70}?'
                r'(?:атаковал|атакован|подвергся|поражён|поражен)|'
                # «атака дронов НА склады» - предлог и множественное число
                r'(?:атак\w*|удар\w*|нал[её]т\w*)\s+[^.]{0,30}?на\s+'
                r'(?:склад|центр|комплекс|объект|хаб|терминал)\w*|'
                # «атаковали сразу несколько складов» - объект отделён
                # от глагола количественным оборотом
                r'атаковал\w*\s+[^.]{0,30}?'
                r'(?:склад|центр|комплекс|объект|хаб|терминал)\w*',
                _re.I)
