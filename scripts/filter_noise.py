...
MANUAL_EXCLUDE = [
    ...
    # Криминальное насилие без политического актора: домен geopolitics присвоен
    # по слову killed в английском оригинале (RU-текст домена не даёт вовсе,
    # замер 23.09.2026). Единичное преступление не является системным сигналом:
    # нет цепочки эскалации, нет переноса в другие домены, нет повторяемости.
    ("массовая стрельба на вечеринке в Южной Африке",
     "криминальное насилие, не геополитика: домен получен по слову killed"),
    # Тот же класс, 24.09.2026. Реестр отнёс запись к «Военным ударам» по слову
    # «нападение»: список исключений в правиле перечисляет «нападение с ножом»,
    # но обратного порядка слов, «ножевое нападение», не покрывал. Порядок слов
    # в правиле исправлен отдельно. Сама запись остаётся вне ленты: нападение
    # без названного актора и мотива не является системным сигналом, а вес 75
    # («Высокий риск») для одного погибшего ставит её вровень с ударами по
    # энергетике. Если следствие назовёт мотив, запись вернётся как терроризм.
    ("ножевое нападение в бенедиктинском аббатстве",
     "насилие без названного актора: реестр отнёс к военным ударам по слову «нападение»"),
    # 26.09.2026. Бытовое насилие в шоу-бизнесе, инцидент 2019 года, пересказ
    # со слов бывшего сотрудника. В ленту запись попала по телу, а не по
    # заголовку: сам заголовок даёт COMMENTARY, событийный класс пришёл от
    # слова «пострадав» во фразе «Пострадавшему пришлось пришивать часть тела».
    # Тот же механизм, что у метки подтверждённости: тело перебивает главное
    # утверждение записи.
    # Вес 52 — лексический дефолт новостного маршрута, ни одно правило не
    # сработало (rules_count = 0), домен «социум» унаследован от канала.
    # Системным сигналом не является: нет актора, нет цепочки, нет переноса
    # в другие домены, а датируется событие 2019 годом при дате записи 23.09.2026.
    ("джиган откусил кусок уха",
     "бытовое насилие в шоу-бизнесе, инцидент 2019 года: событийный класс получен по слову «пострадав» из тела"),
]
