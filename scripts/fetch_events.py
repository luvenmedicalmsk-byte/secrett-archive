...
def _is_news_not_signal(title, summary, domain):
    ...
    _fluff = any(w in b for w in (
        ...
        'отвечает на ваши вопросы','размышления о','размышлен','рассужден','колонка:','колумнист','авторская колонка','профессор кафедры','мнение:',' эссе','почему я ',' weekend','уикенд','деньги в эфире','наш max','лонгрид','на хабре','зацените','почитайт','дайджест','подборка новост','кто управляет','кто стоит за','кто такие','как устроен','как работает'))
    _local = (
