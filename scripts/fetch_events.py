...
def _reclass_domain(title, summary, current):
    ...
    if current in ('economy','social') and any(w in b for w in ('сертификат безопасн','отзыв сертификат','удостоверяющ центр','usb-червь','компьютерн червь','сетев червь','шифровальщик','вредоносн','малвар','кибератак','уязвимост','эксплойт','ботнет','фишинг','троян')):
        return 'technology'
