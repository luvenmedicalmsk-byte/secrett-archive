...
_CHAN_SIGNATURE = re.compile(
    ...
    # «ЕКБ ЧП в TG», «Тюмень ЧП в МАХ» — формат региональных каналов: <бренд> в <площадка>.
    # Ловим по площадке: любые 1-3 слова перед «в TG/МАХ/VK» — это подпись, не текст.
    r'(?:\S+\s+){0,3}в\s+(?:tg|тг|max|мах|макс|vk|вк|дзен|ок)\b|'
...
def _tg_classify(text):
    ...
    LEX = {
        ...
        'climate': ['пожар','наводнен','паводок','ураган','шторм','засух','жара','погод',
                    ...
                    'гибель рыб','мор рыб','замор','загрязнен','разлив нефт','нефтеразлив',
    ...
    if _SIGNAL_ENRICHER_AVAILABLE:
        try:
            ...
            try:
                ...
                _CANARY_DOMAINS = {'climate', 'economy', 'geopolitics', 'technology'}
                _prev_sig = []
                try:
                    import os as _os2
