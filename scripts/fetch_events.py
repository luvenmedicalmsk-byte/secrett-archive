...
def estimate_severity(title, desc, bias=0, weight=1.0):
    ...
    # РЕТРОСПЕКТИВНЫЙ УЩЕРБ (01.09.2026). Сплошная проверка ленты нашла:
    # «Ранее предупреждения о вулканах могут означать больше ложных тревог»
    # получило 90/100, потому что в теле упомянуто извержение 2019 года с
    # 22 погибшими. Материал разбирает методику оповещения, а вес взят у
    # катастрофы семилетней давности.
    # Признак: жертвы или разрушения названы вместе с прошедшим годом, а сам
    # заголовок событийной лексики не содержит. Потолок 55: материал остаётся
    # в ленте, но не конкурирует с сегодняшними происшествиями.
    if (_RETRO_DAMAGE_RE.search(text) and not _SIC_EVENT.search((title or '').lower())
            and _sev_out > 55):
        _sev_out = 55
