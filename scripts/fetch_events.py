...
def _clean_title(raw):
    ...
    msen = re.match(r'^(.{40,160}?[.!?])(?:\s|$)', cand)
    if msen:
        cand = msen.group(1)
    elif len(cand) > 160:
        cand = cand[:160].rsplit(' ', 1)[0].rstrip(' ,;:\u2014\u2013-') + '\u2026'
    return cand.strip()
...
# --- Детерминированная атрибуция субъектов РФ (фолбэк, когда country-LLM вернул GLOBAL/пусто) ---
_RU_SUBJECTS = {
 ...
 'ленинградск':'Ленинградская область','липецк':'Липецкая область','магаданск':'Магаданская область',
 'московск':'Московская область','мурманск':'Мурманская область','нижегородск':'Нижегородская область',
 'новгородск':'Новгородская область','новосибирск':'Новосибирская область','омск':'Омская область',
 'оренбургск':'Оренбургская область','орловск':'Орловская область','пензенск':'Пензенская область',
 'псковск':'Псковская область','ростовск':'Ростовская область','рязанск':'Рязанская область',
 'самарск':'Самарская область','саратовск':'Саратовская область','сахалинск':'Сахалинская область',
 'свердловск':'Свердловская область','смоленск':'Смоленская область','тамбовск':'Тамбовская область',
 'тверск':'Тверская область','томск':'Томская область','тульск':'Тульская область','тюменск':'Тюменская область',
 'ульяновск':'Ульяновская область','челябинск':'Челябинская область','ярославск':'Ярославская область',
 'амурск':'Амурская область','архангельск':'Архангельская область','астраханск':'Астраханская область',
 ...
 'москв':'Москва','санкт-петербург':'Санкт-Петербург','петербург':'Санкт-Петербург','севастопол':'Севастополь',
 'татарстан':'Татарстан','башкорт':'Башкортостан','башкири':'Башкортостан','чуваши':'Чувашия','удмурт':'Удмуртия',
 'мордови':'Мордовия','марий эл':'Марий Эл','коми':'Республика Коми','карели':'Карелия','калмыки':'Калмыкия',
 'буряти':'Бурятия','хакаси':'Хакасия','дагестан':'Дагестан','чечн':'Чечня','чечен':'Чечня','ингушет':'Ингушетия',
 'якути':'Якутия','саха (яку':'Якутия','крым':'Крым','ямал':'ЯНАО','ханты':'ХМАО','чукот':'Чукотка',
}
def ru_subject_in(text):
    """Возвращает каноничное имя субъекта РФ, если он явно упомянут в тексте, иначе None."""
    if not text or not isinstance(text, str): return None
    low = text.lower()
    for stem, name in _RU_SUBJECTS.items():
        if stem in low: return name
    return None
...
def process_events(raw_items):
    ...
    for item in raw_items:
        ...
        _fluff = any(w in _blob for w in (
            'нба','nba','нхл','нфл','ufc','плей-офф','лига чемпионов','чемпионат мира','олимпийск иг','кубок гагарина','knicks',
            'mrbeast','млн подписчиков','подписчиков на youtube','ютубер','тиктокер','инфлюенсер',
            'подарки на день','подарки ко дню','ко дню отца','ко дню матери','что подарить','в стиле роскоши','гид по подаркам','распродаж','чёрная пятниц','черная пятниц',
            'отвечает на ваши вопросы','размышления о','размышлен','рассужден','колонка:','колумнист','авторская колонка','профессор кафедры','мнение:',' эссе','почему я ',' weekend','уикенд','деньги в эфире','наш max','лонгрид','на хабре','зацените','почитайт','дайджест','подборка новост','кто управляет','кто стоит за','кто такие','как устроен','как работает','за информацию о','объявило вознаграждение','объявила вознаграждение','liv golf','ежедневная записка','еженедельный обзор','смогут ли','что это значит','выпустил ролик','выпустила ролик','вирусн трек','вирусный трек','финансовую грамотность','грамотность включ','повысил кредитный рейтинг','повысило кредитный рейтинг','подтвердил кредитный рейтинг','школьного питания','школьное питание','зазывают в','астролог','гороскоп','нумеролог','по знаку зодиака','ищут ответы у астролог','знаки зодиака','карты таро','для здоровья','для здоров','вредно ли','полезно ли','похуден','рацион питан','диет ','рецепт','главное из','главное за','коротко о главном','итоги дня','итоги недели','дуб робин','раскопа','археолог','имперск вилл','панорам оборон','робопёс','робопес','каштан','инклюзивн мер','развлекать гост'))
    ...
    _before_ns = len(top_events); top_events = _keep_ns
...
def _tg_classify(text):
    ...
    LEX = {
        ...
        'social': ['вспышк','заболеваем','инфекци','эпидеми','вирус','корь','грипп','лихорадк','карантин','госпитализац',
                   'сокращени','увольнени','задолженност','забастовк','занятост','дефицит врач','закрыт больниц',
