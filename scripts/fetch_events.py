...
def _title_polish(s):
    """Финальная полировка заголовка: лайв-блог, расклейка кир/лат, перевод англ. погодных терминов."""
    if not s: return s
    ...
    if re.search(r'(в\s+прямом\s+эфире|прямая\s+трансляция|прямой\s+эфир|онлайн-трансляция)\s*:', s, re.I):
        ...
        s = _LIVE_RE.sub(' ', s)
    # 2) расклейка кириллица<->латиница (из-заheatwave -> из-за heatwave)
    s = re.sub(r'([а-яёА-ЯЁ])([A-Za-z])', r'\1 \2', s)
    s = re.sub(r'([A-Za-z])([а-яёА-ЯЁ])', r'\1 \2', s)
    # 3) перевод оставшихся англ. погодных терминов
    for en, ru in sorted(_EN_WX.items(), key=lambda kv:-len(kv[0])):
...
'замор','разлив','загрязн','токсичн','эколог','заморозк','штормов','потоп','град','стих','бедств',
'катастроф','море','моря','озер','рек','водоём','водоем','уровень вод','деград','экосистем','побережь',
'берег','акватор','залив','пролив','атмосфер','почв','грунт','сейсм','магнитуд','ветер','ветра','гроза',
'мороз','холод','потеплени','похолодани','приливн','нагон','размыв','эрози','опустын','вырубк','лес',
...
_GATE_DROP_RX = re.compile(
    ...
    r'похорон|прощание с|последнее прощание|скончал|умер в возраст|погребени|траурн\w* церемони|кто присутствует)', re.I)
...
_GATE_RESCUE_RX = re.compile(
    ...
    r'наводнен|землетряс|засух|ураган|пожар\w* охватил|извержен)', re.I)
...
_SHA_EVENT = re.compile(r'(удар|атак|обстрел|сбит|сбил|уничтож|разрушен|поврежд|взрыв|пожар|возгоран|'
    ...
    r'ввёл\w* ограничен|ввел\w* ограничен)', re.I)
...
_SHA_DOMV = {
    'geopolitics': r'войн|военн|ракет|дрон|бпла|санкц|нато|удар|обстрел|границ|теракт|оккупац|нпз|наступлен|пораж|взрыв|нападени|всу|пво|столкновени|стрельб',
    'economy': r'инфляц|дефолт|банкрот|обвал|бирж|нефт|рубл|бензин|топлив|азс|ставк|рухнул|акци|котировк|фондов|цен\w|подорожал|баррел|стоимост',
    ...
    'climate': r'наводн|паводок|землетряс|шторм|ураган|тайфун|засух|пожар|вулкан|цунами|оползен|циклон|торнадо|морск\w* л[её]д|подтопл|\bсель\b|деград\w* (?:каспи|мор|озер)|маловод|аномальн\w* (?:жар|температур)|гроза|опасн\w* осадк',
}
