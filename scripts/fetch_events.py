...
_SIG_RE = re.compile(
    r'войн|военн|армия|армии|войск|ракет|дрон|бпла|обстрел|санкц|эмбарго|пошлин|тариф|нато|оон|'
    ...
    r'нефт|газ|баррел|бюджет|дефицит|госдолг|ипотек|экспорт|импорт|производств|увольнен|сокращен|'
    r'подорожан|рубл|доллар|евро|юан|валют|бирж|котировк|акци|облигац|инвест|энерг|recession|'
...
_DOMAIN_VOCAB = {
    ...
    'technology': ('кибератак','хакер','взлом','утечк','уязвим','cve','ddos','вредоносн','malware','дата-центр','цод','облач','спутник','глонасс','gps','нейросет','искусственн интеллект','gmail','телеком','оператор связи','отключ интернет','энергосет','подстанц','платёжн','авторизац','сервер','ядерн','радиац','аэс','реактор'),
...
_CFM_EXPECT = ('ожидается','планируется','может привести','возможно','не исключен','рассматрива','предполага','как ожидается','планирует')
_CFM_NEGOT = ('переговор','обсужда','ведут диалог','на стадии','консультаци','стороны обсуждают','раунд перегов','диалог о')
_CFM_PRELIM = ('предварительн','проект соглашения','меморандум','рамочн','договорённость о намерениях','согласовали рамки','согласованы рамки','близки к соглашению','приблизились к соглашению')
_CFM_STATE = ('заявил','сообщил о намерении','призвал','пригрозил','предложил','выразил готовность','выступил с инициатив')
_CFM_GRAND = ('достигли соглашения','достигли договорённости','договорились о мире','договорились о прекращении','подписано соглашение','подписали соглашение','историческое соглашение','соглашение о прекращении войны','соглашение о прекращении конфликта','пролив открыт','открыт ормузский','объявили о мире')
def _confirm_level(text):
    ...
    if any(w in b for w in _CFM_GRAND): return 'preliminary'
    return 'confirmed'
