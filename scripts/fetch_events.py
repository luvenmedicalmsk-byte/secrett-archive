...
def process_events(raw_items):
    ...
    for item in raw_items:
        ...
        if any(k in _ttl0 for k in ('отчет о ситуации','отчёт о ситуации','situation report','sitrep','период отчетности','reporting period','cluster report')):
            _LOSS['sev']+=1; continue
        # S41: «сигнал или новость» -- безусловный дроп не-сигналов (severity не спасает).
        # Категории: локальные ЧП/криминал, гражданские транспортные аварии, бытовые взрывы газа,
        # спорт/селебрити/лайфстайл/колонки. Военные и террор-сигналы защищены _mil-гейтом.
        _blob = _ttl0 + ' ' + str(item.get('desc','')).lower()
        _mil = any(w in _blob for w in ('теракт','стрельб','захват заложник','боевик','диверс','обстрел','ракет','дрон','бпла','всу','пво','сбит','военн','снаряд','боеприпас'))
        # 1) локальные ЧП / криминал / атаки животных
        _local = (
        ...
        # 2) гражданские авиа/транспортные аварии (не сбитые)
        ...
        # 3) бытовые взрывы газа (не боеприпас/диверсия)
        ...
        # 4) спорт / селебрити / лайфстайл / колонки -- не сигнал риска
        ...
        if not _mil and (_local or _accident or _gas or _fluff):
            _LOSS['sev']+=1; continue
