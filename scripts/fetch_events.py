...
def fetch_global_structural_risks():
    """Структурные риски по всем странам и регионам мира — авторская аналитика Архива.
    Постоянный слой карты: геополитика, экономика, климат, технологии, социум."""
    today = datetime.now(timezone.utc).strftime('%Y-%m-%d')
    risks = [

        # ── СЕВЕРНАЯ АМЕРИКА ──────────────────────────────────────────────────
        {'title': 'США: долговой потолок и бюджетный дефицит', 'domain': 'economy',
         'severity': 82, 'lat': 38.9, 'lng': -77.0, 'region': 'США',
         'summary': 'Государственный долг США превысил $36 трлн. Растущие расходы на обслуживание долга ограничивают бюджетные возможности и создают риск долгового кризиса при повышении ставок.'},
        {'title': 'США: пузырь активов в секторе ИИ и недвижимости', 'domain': 'economy',
         'severity': 78, 'lat': 37.7, 'lng': -122.4, 'region': 'США',
         'summary': 'Оценки ИИ-компаний достигли исторических максимумов. Перегрев рынка недвижимости в крупных городах формирует риск резкой коррекции при изменении монетарной политики.'},
        {'title': 'США: геоэкономическое противостояние с Китаем', 'domain': 'geopolitics',
         'severity': 88, 'lat': 40.7, 'lng': -74.0, 'region': 'США',
         'summary': 'Торговые ограничения, инвестиционный скрининг и технологические санкции углубляют разрыв между двумя крупнейшими экономиками. Риск эскалации в 2026-2028.'},
        {'title': 'США: дезинформация и поляризация общества', 'domain': 'social',
         'severity': 75, 'lat': 39.0, 'lng': -95.0, 'region': 'США',
         'summary': 'Социальная поляризация достигла исторических максимумов. Дипфейки и ИИ-генерированный контент усиливают информационные войны внутри страны.'},
        {'title': 'Мексика: контроль наркокартелей над территориями', 'domain': 'social',
         'severity': 79, 'lat': 23.6, 'lng': -102.5, 'region': 'Мексика',
         'summary': 'Картели контролируют значительные территории страны, подрывая государственные институты. Уровень насилия остаётся критически высоким в приграничных регионах.'},
        {'title': 'Канада: зависимость от экспорта в США', 'domain': 'economy',
         'severity': 65, 'lat': 45.4, 'lng': -75.7, 'region': 'Канада',
         'summary': 'Около 75% канадского экспорта направляется в США. Торговые тарифы и протекционизм создают системную уязвимость для канадской экономики.'},

        # ── ЛАТИНСКАЯ АМЕРИКА ─────────────────────────────────────────────────
        {'title': 'Аргентина: гиперинфляция и долговой кризис', 'domain': 'economy',
         'severity': 85, 'lat': -34.6, 'lng': -58.4, 'region': 'Аргентина',
         'summary': 'Инфляция остаётся трёхзначной. Реструктуризация долга и шоковая либерализация создают социальную напряжённость. Риск нового дефолта сохраняется.'},
        {'title': 'Бразилия: вырубка Амазонии и экосистемный коллапс', 'domain': 'climate',
         'severity': 83, 'lat': -3.1, 'lng': -60.0, 'region': 'Бразилия',
         'summary': 'Темпы вырубки Амазонии угрожают переломным точкам экосистемы. Деградация лесов ускоряет изменение климата и разрушает водные циклы Южной Америки.'},
        {'title': 'Венесуэла: государственный коллапс и миграционный кризис', 'domain': 'social',
         'severity': 86, 'lat': 10.5, 'lng': -66.9, 'region': 'Венесуэла',
         'summary': 'Более 7 млн венесуэльцев покинули страну. Экономика не восстановилась, политический кризис продолжается. Региональная дестабилизация нарастает.'},
        {'title': 'Колумбия: мирный процесс под угрозой', 'domain': 'geopolitics',
         'severity': 68, 'lat': 4.7, 'lng': -74.1, 'region': 'Колумбия',
         'summary': 'Переговоры с вооружёнными группировками зашли в тупик. Производство коки достигло рекордных уровней. Нарастает риск возобновления масштабного конфликта.'},
        {'title': 'Перу: политическая нестабильность и горнодобыча', 'domain': 'social',
         'severity': 67, 'lat': -12.0, 'lng': -77.0, 'region': 'Перу',
         'summary': 'Частая смена президентов подрывает институциональную стабильность. Конфликты вокруг горнодобывающих проектов обостряют социальную напряжённость.'},

        # ── ЕВРОПА ────────────────────────────────────────────────────────────
        {'title': 'Германия: деиндустриализация и рецессия', 'domain': 'economy',
         'severity': 80, 'lat': 52.5, 'lng': 13.4, 'region': 'Германия',
         'summary': 'Промышленное производство снижается третий год подряд. Высокие энергозатраты вытесняют производство за рубеж. Риск структурной рецессии крупнейшей экономики ЕС.'},
        {'title': 'Франция: социальная поляризация и политический кризис', 'domain': 'social',
         'severity': 72, 'lat': 48.9, 'lng': 2.3, 'region': 'Франция',
         'summary': 'Политическое пространство фрагментировано. Протестные движения регулярно парализуют страну. Реформы пенсионной и иммиграционной систем вызывают острые конфликты.'},
        {'title': 'Великобритания: постбрекситовая нестабильность', 'domain': 'economy',
         'severity': 69, 'lat': 51.5, 'lng': -0.1, 'region': 'Великобритания',
         'summary': 'Торговые барьеры с ЕС продолжают сдерживать рост. Инфляция остаётся выше целевых показателей. Государственные финансы испытывают давление.'},
        {'title': 'Польша: форпост НАТО на восточном фланге', 'domain': 'geopolitics',
         'severity': 74, 'lat': 52.2, 'lng': 21.0, 'region': 'Польша',
         'summary': 'Польша наращивает военные расходы до 4% ВВП. Риск эскалации у восточных границ НАТО остаётся высоким. Инфраструктура становится стратегическим активом.'},
        {'title': 'Венгрия: эрозия верховенства права в ЕС', 'domain': 'geopolitics',
         'severity': 66, 'lat': 47.5, 'lng': 19.1, 'region': 'Венгрия',
         'summary': 'Систематическое ослабление независимости судов и СМИ. Блокирование решений ЕС по Украине создаёт институциональный кризис внутри союза.'},
        {'title': 'Турция: геополитический балансир и инфляционная ловушка', 'domain': 'economy',
         'severity': 73, 'lat': 39.9, 'lng': 32.9, 'region': 'Турция',
         'summary': 'Инфляция остаётся двузначной, несмотря на ужесточение ДКП. Турция использует своё положение между Западом и Россией как геополитический рычаг.'},
        {'title': 'Сербия: балканский узел напряжённости', 'domain': 'geopolitics',
         'severity': 70, 'lat': 44.8, 'lng': 20.5, 'region': 'Сербия',
         'summary': 'Напряжённость в Косово не урегулирована. Сербия балансирует между ЕС и Россией. Регион остаётся зоной потенциальной эскалации конфликтов.'},

        # ── РОССИЯ И СНГ ──────────────────────────────────────────────────────
        {'title': 'Россия: структурные риски военной экономики', 'domain': 'economy',
         'severity': 84, 'lat': 55.75, 'lng': 37.6, 'region': 'Россия',
         'summary': 'Перевод экономики на военные рельсы создаёт долгосрочные структурные дисбалансы. Санкционное давление ограничивает технологический импорт и финансовые инструменты.'},
        {'title': 'Россия: конфликт на Украине — риск эскалации', 'domain': 'geopolitics',
         'severity': 92, 'lat': 50.4, 'lng': 30.5, 'region': 'Украина',
         'summary': 'Продолжающийся конфликт несёт риск дальнейшей эскалации, включая применение нестратегического ядерного оружия. Гуманитарные и экономические последствия охватывают весь регион.'},
        {'title': 'Казахстан: ресурсная зависимость и транзит власти', 'domain': 'geopolitics',
         'severity': 65, 'lat': 51.2, 'lng': 71.4, 'region': 'Казахстан',
         'summary': 'Экономика критически зависит от нефтегазового экспорта. Балансирование между Россией, Китаем и Западом создаёт геополитическую уязвимость.'},
        {'title': 'Беларусь: политический кризис и зависимость от России', 'domain': 'geopolitics',
         'severity': 71, 'lat': 53.9, 'lng': 27.6, 'region': 'Беларусь',
         'summary': 'Режим сохраняет власть через репрессии. Экономическая и военная зависимость от России нарастает. Страна фактически стала плацдармом для операций против Украины.'},

        # ── БЛИЖНИЙ ВОСТОК ────────────────────────────────────────────────────
        {'title': 'Израиль-Иран: риск прямого военного столкновения', 'domain': 'geopolitics',
         'severity': 89, 'lat': 32.1, 'lng': 34.8, 'region': 'Израиль',
         'summary': 'Взаимные удары между Израилем и Ираном вышли за рамки прокси-конфликта. Ядерная программа Ирана приближается к критическим порогам. Риск региональной войны высок.'},
        {'title': 'Газа и Западный берег: гуманитарная катастрофа', 'domain': 'social',
         'severity': 90, 'lat': 31.5, 'lng': 34.5, 'region': 'Палестина',
         'summary': 'Масштабный гуманитарный кризис с сотнями тысяч перемещённых. Риск распространения конфликта на Ливан, Иорданию и другие страны региона.'},
        {'title': 'Саудовская Аравия: нефтяная геополитика и трансформация', 'domain': 'economy',
         'severity': 72, 'lat': 24.7, 'lng': 46.7, 'region': 'Саудовская Аравия',
         'summary': 'Нефтяная монархия форсирует диверсификацию экономики через Vision 2030. Баланс между ОПЕК+ и западными партнёрами определяет мировые цены на нефть.'},
        {'title': 'Йемен: затяжной конфликт и хуситская угроза', 'domain': 'geopolitics',
         'severity': 82, 'lat': 15.4, 'lng': 44.2, 'region': 'Йемен',
         'summary': 'Хуситы атакуют торговое судоходство в Красном море, нарушая глобальные цепочки поставок. Гуманитарный кризис не ослабевает.'},
        {'title': 'Ирак: нестабильность и иранское влияние', 'domain': 'geopolitics',
         'severity': 74, 'lat': 33.3, 'lng': 44.4, 'region': 'Ирак',
         'summary': 'Проиранские группировки укрепляют влияние. Иностранные военные базы становятся мишенями атак. Нефтяная инфраструктура уязвима для диверсий.'},
        {'title': 'Сирия: постконфликтная нестабильность', 'domain': 'social',
         'severity': 76, 'lat': 33.5, 'lng': 36.3, 'region': 'Сирия',
         'summary': 'Фрагментация страны сохраняется. Миллионы беженцев не могут вернуться. Экономика разрушена, риски возобновления масштабных столкновений остаются.'},

        # ── АФРИКА ────────────────────────────────────────────────────────────
        {'title': 'Сахель: распространение джихадистских группировок', 'domain': 'geopolitics',
         'severity': 85, 'lat': 14.0, 'lng': -1.5, 'region': 'Сахель (Африка)',
         'summary': 'Мали, Буркина-Фасо, Нигер охвачены вооружёнными конфликтами. Уход западных сил и приход российских ЧВК меняет баланс сил. Гуманитарный кризис нарастает.'},
        {'title': 'ДРК: конфликт на востоке и кобальтовая зависимость', 'domain': 'geopolitics',
         'severity': 81, 'lat': -4.3, 'lng': 15.3, 'region': 'ДР Конго',
         'summary': 'Вооружённый конфликт на востоке страны контролирует добычу кобальта — ключевого материала для батарей электромобилей. Глобальные цепочки поставок критически зависимы.'},
        {'title': 'Нигерия: экономический кризис крупнейшей экономики Африки', 'domain': 'economy',
         'severity': 76, 'lat': 9.1, 'lng': 7.4, 'region': 'Нигерия',
         'summary': 'Девальвация найры, высокая инфляция и бандитизм создают системный кризис. Нигерия — крупнейшая экономика Африки — балансирует на грани.'},
        {'title': 'Эфиопия: постконфликтная нестабильность и голод', 'domain': 'social',
         'severity': 78, 'lat': 9.0, 'lng': 38.7, 'region': 'Эфиопия',
         'summary': 'После конфликта в Тыграе страна восстанавливается медленно. Засухи и продовольственная нестабильность угрожают миллионам. Риск новых вооружённых столкновений остаётся.'},
        {'title': 'Судан: гражданская война и гуманитарная катастрофа', 'domain': 'social',
         'severity': 88, 'lat': 15.6, 'lng': 32.5, 'region': 'Судан',
         'summary': 'Война между армией и ССБ привела к одному из крупнейших кризисов перемещения в мире. Свыше 10 млн человек покинули дома. Голод охватывает целые регионы.'},
        {'title': 'ЮАР: энергетический кризис и безработица', 'domain': 'economy',
         'severity': 73, 'lat': -26.2, 'lng': 28.0, 'region': 'ЮАР',
         'summary': 'Отключения электроэнергии (loadshedding) продолжают тормозить экономику. Безработица среди молодёжи превышает 60%. Социальная нестабильность нарастает.'},
        {'title': 'Сомали: пиратство, терроризм и несостоявшееся государство', 'domain': 'geopolitics',
         'severity': 77, 'lat': 2.0, 'lng': 45.3, 'region': 'Сомали',
         'summary': 'Аш-Шабааб контролирует значительные территории. Пиратство в Индийском океане угрожает морским путям. Государственные институты слабы.'},

        # ── ЮЖНАЯ И ЦЕНТРАЛЬНАЯ АЗИЯ ──────────────────────────────────────────
        {'title': 'Афганистан: гуманитарный коллапс под властью талибов', 'domain': 'social',
         'severity': 84, 'lat': 34.5, 'lng': 69.2, 'region': 'Афганистан',
         'summary': 'Экономика рухнула после прихода талибов. Полная изоляция женщин от образования и труда. Голод угрожает миллионам. Страна — источник нестабильности для региона.'},
        {'title': 'Пакистан: ядерная держава на грани банкротства', 'domain': 'economy',
         'severity': 83, 'lat': 33.7, 'lng': 73.1, 'region': 'Пакистан',
         'summary': 'Пакистан балансирует на грани дефолта при ядерном арсенале. Высокая инфляция, долговой кризис и политическая нестабильность создают опасный коктейль.'},
        {'title': 'Индия: водный стресс и экстремальная жара', 'domain': 'climate',
         'severity': 82, 'lat': 20.6, 'lng': 78.9, 'region': 'Индия',
         'summary': 'Температурные рекорды фиксируются ежегодно. Подземные воды истощаются критически быстро в аграрных штатах. Риски для продовольственной безопасности 1,4 млрд человек.'},
        {'title': 'Индия-Китай: пограничная напряжённость в Гималаях', 'domain': 'geopolitics',
         'severity': 74, 'lat': 33.9, 'lng': 78.0, 'region': 'Индия-Китай',
         'summary': 'Противостояние на линии Макмагона периодически обострятся. Военное присутствие обеих ядерных держав в регионе нарастает. Риск инцидента высок.'},
        {'title': 'Бангладеш: климатическая уязвимость дельты', 'domain': 'climate',
         'severity': 80, 'lat': 23.7, 'lng': 90.4, 'region': 'Бангладеш',
         'summary': 'Страна — в числе наиболее уязвимых к повышению уровня моря. Регулярные циклоны и наводнения угрожают десяткам миллионов. К 2050 треть территории может быть затоплена.'},

        # ── ВОСТОЧНАЯ АЗИЯ ────────────────────────────────────────────────────
        {'title': 'Тайваньский пролив: риск военного конфликта', 'domain': 'geopolitics',
         'severity': 91, 'lat': 23.7, 'lng': 120.9, 'region': 'Тайвань',
         'summary': 'Китай усиливает военное давление на Тайвань. Полупроводниковая промышленность острова критична для мировой экономики. Конфликт здесь остановит глобальное производство электроники.'},
        {'title': 'Китай: экономическое замедление и долговой пузырь недвижимости', 'domain': 'economy',
         'severity': 85, 'lat': 39.9, 'lng': 116.4, 'region': 'Китай',
         'summary': 'Кризис недвижимости (Evergrande и другие) не преодолён. Дефляционное давление и замедление роста создают риски для мировой экономики. Внутреннее потребление не восстановилось.'},
        {'title': 'КНДР: ядерные испытания и ракетные угрозы', 'domain': 'geopolitics',
         'severity': 83, 'lat': 39.0, 'lng': 125.8, 'region': 'КНДР',
         'summary': 'Северная Корея продолжает развивать ядерный и ракетный потенциал. Военное сотрудничество с Россией меняет региональный баланс сил. Провокации учащаются.'},
        {'title': 'Япония: демографический коллапс и долг', 'domain': 'social',
         'severity': 71, 'lat': 35.7, 'lng': 139.7, 'region': 'Япония',
         'summary': 'Население сокращается рекордными темпами. Государственный долг превышает 260% ВВП. Нормализация монетарной политики создаёт риски для глобальных рынков облигаций.'},
        {'title': 'Южно-Китайское море: территориальные споры', 'domain': 'geopolitics',
         'severity': 79, 'lat': 15.0, 'lng': 115.0, 'region': 'Южно-Китайское море',
         'summary': 'Китай продолжает строительство искусственных островов. Инциденты с Филиппинами и Вьетнамом учащаются. Риск столкновения с участием США нарастает.'},

        # ── ЮГО-ВОСТОЧНАЯ АЗИЯ ────────────────────────────────────────────────
        {'title': 'Мьянма: военный переворот и гражданская война', 'domain': 'geopolitics',
         'severity': 82, 'lat': 19.7, 'lng': 96.1, 'region': 'Мьянма',
         'summary': 'Хунта не контролирует значительную часть страны. Этнические вооружённые группировки наступают. Гуманитарный кризис охватывает миллионы людей.'},
        {'title': 'Индонезия: климатические риски архипелага', 'domain': 'climate',
         'severity': 75, 'lat': -6.2, 'lng': 106.8, 'region': 'Индонезия',
         'summary': 'Джакарта тонет — столица переносится. Повышение уровня моря и тропические циклоны угрожают 17 000 островам. Вулканическая активность создаёт дополнительные риски.'},

        # ── ГЛОБАЛЬНЫЕ РИСКИ ──────────────────────────────────────────────────
        {'title': 'Глобально: кризис критической инфраструктуры', 'domain': 'technology',
         'severity': 83, 'lat': 51.5, 'lng': -0.1, 'region': 'Глобально',
         'summary': 'Устаревшая инфраструктура (энергосети, трубопроводы, кабели) становится уязвимой точкой. Экстремальные погодные явления и кибератаки создают каскадные сбои.'},
        {'title': 'Глобально: квантовые угрозы криптографии', 'domain': 'technology',
         'severity': 76, 'lat': 47.4, 'lng': 8.5, 'region': 'Глобально',
         'summary': 'Квантовые компьютеры приближаются к уровню, способному взломать текущие стандарты шифрования. Финансовая система, военные коммуникации и персональные данные под угрозой к 2030-2035.'},
        {'title': 'Глобально: продовольственная безопасность под давлением', 'domain': 'social',
         'severity': 80, 'lat': 0.0, 'lng': 20.0, 'region': 'Глобально',
         'summary': 'Экстремальная жара, засухи и конфликты нарушают производство продовольствия. Более 700 млн человек страдают от голода. Ценовые шоки дестабилизируют развивающиеся страны.'},
        {'title': 'Глобально: неравенство как системный усилитель рисков', 'domain': 'social',
         'severity': 81, 'lat': 0.0, 'lng': 0.0, 'region': 'Глобально',
         'summary': 'Неравенство является самым взаимосвязанным глобальным риском. Усиливает социальную поляризацию, политическую нестабильность и уязвимость к экономическим потрясениям.'},
        {'title': 'Глобально: дезинформация и дипфейки на выборах', 'domain': 'technology',
         'severity': 79, 'lat': 48.8, 'lng': 2.3, 'region': 'Глобально',
         'summary': 'ИИ-генерированные дипфейки и микротаргетированная дезинформация подрывают демократические процессы. В 2026 году выборы пройдут в десятках стран с риском манипуляций.'},
        {'title': 'Красное море: угрозы судоходству хуситов', 'domain': 'economy',
         'severity': 82, 'lat': 15.0, 'lng': 42.0, 'region': 'Красное море',
         'summary': 'Атаки хуситов вынудили крупнейшие судоходные компании обходить Суэцкий канал. Глобальные цепочки поставок удлинились на 2-3 недели, рост затрат на логистику.'},
        {'title': 'Арктика: таяние льдов и геополитическая гонка', 'domain': 'climate',
         'severity': 78, 'lat': 80.0, 'lng': 30.0, 'region': 'Арктика',
         'summary': 'Таяние арктического льда открывает новые судоходные маршруты и месторождения. Россия, США, Китай и Скандинавия усиливают военное и экономическое присутствие в регионе.'},
    ]

    events = []
    for r in risks:
        ev_id = 'gs' + __import__('hashlib').md5(r['title'].encode()).hexdigest()[:8]
        events.append({
            'id': ev_id,
            'title': r['title'],
            'domain': r['domain'],
