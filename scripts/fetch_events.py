...
_PROMO_RE = re.compile(
    ...
    r'(?:\s+\u043d\u0430\b[^.!?\n]{0,60})?[.!\s]*$)',
    re.IGNORECASE)
# Подписи каналов в хвосте текста (02.09.2026). Регулярное выражение было
# регистрозависимым, и «Прямой Эфир» с заглавной Э не чистилось: в ленте
# такой хвост встретился двенадцать раз. Добавлен флаг re.I и подписи,
# найденные сплошной проверкой хвостов.
_CHAN_SIG_RE = re.compile(
    # Смешанная кириллица и латиница (04.09.2026): «ММI в Max» пишется через
    # кириллические М и украинскую І, «Мах» тоже кириллицей. Побуквенные
    # классы покрывают оба алфавита: подпись канала снимается независимо от
    # того, какими буквами набрана.
    r'\s*(?:Прямой\s+эфир|Топор\s*Live|[МM][МM][IІi]\s+в\s+[МM][аa][хx]|Mash|Baza|Readovka|'
    r'Осторожно,?\s+новости|Раньше\s+всех|Ну\s+и\s+новости|Подъём|SHOT|'
    r'Военкоры|Рыбарь|Интерфакс\s*·\s*\w+)\s*[.!…]?\s*$', re.I)
...
_PROMO3_RE = re.compile(
    ...
    re.I)
# Хвост-тизер продолжения (02.09.2026). «Какие ещё изменения готовят власти и
# что о них думают крупные потребители энергии — Канал в «Максе» Приложение для
# iOS и Android». Первая часть это анонс платного продолжения, вторая подпись
# приложения. Подписи каналов и приложений снимались, анонс оставался и
# выглядел как оборванная мысль.
_PROMO6_RE = re.compile(
    r'[\s.,\u2014\u2013-]*(?:что|какие|как|почему|зачем|кто)\b[^.!?\n]{0,90}'
    r'(?:читайте|смотрите|узнайте|в\s+материале|в\s+статье|подробнее)[^.!?\n]{0,40}[.!\s]*$'
    r'|[\s.,\u2014\u2013-]*(?:какие|что)\b[^.!?\n]{0,90}\s[\u2014\u2013-]\s*$'
    r'|\s*Приложение\s+для\s+(?:iOS|Android|айфон|андроид)[^.!?\n]{0,30}[.!\s]*$'
    r'|\s*(?:Канал|Чат|Бот)\s+в\s+[«"]?\w+[»"]?[.!\s]*$',
    re.I)
...
def _strip_promo(t):
    ...
    prev = None
    while prev != t and t:
        prev = t; t = _PROMO6_RE.sub('', _PROMO_EN_RE.sub('', _PROMO5_RE.sub('', _PROMO4_RE.sub('', _PROMO3_RE.sub('', _PROMO2_RE.sub('', _CHAN_SIG_RE.sub('', _PROMO_RE.sub('', t)))))))).rstrip(' \t\n|\u00b7\u2014\u2013-')
...
DOMAIN_RULES = {
    "climate": {
        ...
        "keywords": [
            ...
            # --- Гроза и молния (30.08.2026) ---
            # «Молния ударила прямо в женщину на пляже в Бразилии» уходило мимо
            # климата: слов «молния» и «гроза» в словаре не было вовсе, зато
            # «удар» внутри слова «ударила» давало геополитику с весом 1.5.
            # «гроза» как ключ не годится: это подстрока слова «угроза»,
            # которое встречается в военных сводках постоянно. Берём только
            # формы, не входящие в «угроза».
            "молни","грозов","удар молнии","lightning","thunderstorm"
        ],
        "weight": 1.0,
    ...
    "geopolitics": {
        ...
        "exclude": ["flood","wildfire","earthquake","inflation","recession","hack","cyber",
                    # «молния ударила» это не военный удар: корень «удар» давал
                    # геополитике 1.5 против климата 1.0 (30.08.2026).
                    # «гроза» сюда не добавляем: она сидит внутри «угроза» и
                    # снижала бы вес настоящих военных сводок.
                    # «грозовой фронт» — метеорология, но «фронт» даёт
                    # геополитике 1.5 и перебивает климат.
                    "молни","lightning","грозов","атмосферный фронт"]
    },
    "technology": {
...
def detect_domain(title, desc):
    ...
    # «Сортировочный центр Ozon и хаб доставки в Белгороде попали под атаку ВСУ»
    # получало домен climate: единственное попадание словаря — «пожар»,
    # а военный признак не сработал, потому что в списке была только форма
    # «атаковал». Формы «попали под атаку», «подверглись атаке» и само
    # обозначение стороны в списке отсутствовали (30.08.2026).
    _mil_attack = re.search(r'(бпла|беспилотник|дрон|ракет|обстрел|авиауд|удар\w* по|'
                            r'атаковал|под атак\w*|подверг\w*\s+атак\w*|нанес\w*\s+удар|'
                            r'взрыв прогрем|подорвал|теракт|боевик|корвет|'
                            r'военн\w* корабл|снаряд)', text)
    # Кибератака тоже «атака», но её домен — technology. Без этого исключения
    # «сервис подвергся кибератаке» ушло бы в geopolitics.
    if _mil_attack and re.search(r'(кибератак|хакерск|ddos|фишинг|взлом\w*\s+(?:сайт|систем|аккаунт))', text):
        _mil_attack = None
...
# УСТОЙЧИВЫЙ ОТРАСЛЕВОЙ ТРЕНД (30.08.2026).
# «Число иностранных туристов в Турции снижается третий месяц подряд:
# в мае 3,6%, в июне 4%, в июле 0,3%» получало 45 из 100. Формула считает
# по словам разрушения — «погиб», «взрыв», «удар» — и у отраслевого спада
# таких слов нет вовсе. Между тем многомесячное снижение отраслевого
# показателя это структурный сигнал: он не отыгрывается за неделю и
# концентрируется в узлах (для туризма — приморские провинции).
#
# Признак требует ДВУХ условий одновременно: длительность (месяц подряд,
# квартал подряд) и количественную оценку (процент падения). Одного
# слова «снижается» недостаточно — это может быть разовая новость.
_TREND_DURATION_RE = re.compile(
    r'((?:втор|трет|четвёрт|четверт|пят|шест)\w*\s+(?:месяц|квартал)\s+подряд)'
    r'|((?:снижа|сокраща|пада|падени|снижени|сокращени)\w*\s+(?:уже\s+)?'
    r'(?:втор|трет|четвёрт|четверт|пят|шест)\w*\s+(?:месяц|квартал))'
    r'|(?:for\s+(?:a\s+)?(?:third|fourth|fifth)\s+(?:consecutive|straight)\s+month)', re.I)
_TREND_MAGNITUDE_RE = re.compile(
    r'(?:на\s+)?\d+[.,]?\d*\s*%|\d+[.,]?\d*\s*процент', re.I)
# Двузначный ценовой скачок за короткое окно (04.09.2026). «Плюс 10,02% за
# неделю» словарь не видел вовсе: маркеры описывают направление, но не
# величину. Десять процентов за неделю по сырьевому контракту это не
# колебание, а движение режима.
# Требуется одновременно: величина не менее 10% И короткое окно (неделя,
# сутки, торговая сессия), иначе годовой рост попадёт в тот же класс.
_PRICE_JUMP_RE = re.compile(
    r'(?:на\s+)?(1\d|[2-9]\d)[.,]?\d*\s*%[^.]{0,60}'
    r'(?:за\s+(?:недел|сутк|день|торгов\w*\s+сесси)|с\s+начала\s+недели|'
    r'по\s+сравнению\s+с\s+начал\w*\s+недели)'
    r'|(?:за\s+(?:недел|сутк|день)|с\s+начала\s+недели)[^.]{0,60}'
    r'(?:на\s+)?(?:1\d|[2-9]\d)[.,]?\d*\s*%', re.I)
_PRICE_JUMP_FLOOR = 62

# Короткие корни, дающие ложные совпадения внутри других слов.
# «удар» ⊂ «государственный», «атак» ⊂ «атакже» в слитном тексте,
# «бой» ⊂ «оборот», «мина» ⊂ «доминанта», «танк» ⊂ «Танкер» в роли судна.
# Короткие корни требуют границы. Проверено на реальных ловушках:
# «удар» ⊂ госУДАРственный, «war» ⊂ warning, «coup» ⊂ couple,
# «обвал» ⊂ обваловка.
# Два режима: кириллическим корням нужна граница СЛЕВА (окончания
# допустимы: удар, удары, ударов), латинским и «обвал» — с обеих сторон.
_HIGH_LEFT_BOUNDARY = frozenset({'удар', 'война', 'взрыв', 'штурм', 'убит', 'обвал'})
_HIGH_FULL_WORD = frozenset({'war', 'coup'})
# «обвал» проверен отдельно: полнословная граница теряла законные формы
# «обвалился», «обвалились». Оставлен левограничным, ловушка «обваловка»
# (насыпь вокруг резервуара) снимается явным исключением.
_OBVAL_TRAP_RE = re.compile(r'обвалов[кч]', re.I)

_TREND_FLOOR = 60

# Необратимая деградация природной системы: процесс без обратной связи и без
# восстановления в человеческом горизонте. Требуются два условия сразу —
# признак необратимости И природная система, иначе «деградация сервиса»
# попадёт в тот же класс.
_IRREVERSIBLE_RE = re.compile(
    r'деградац\w*|высыхан|усыхан|обмелен|пересых|засолен|закислен'
    r'|обесцвечивани\w*\s+корал|вымиран|исчезновени\w*\s+вид'
    r'|таяни\w*\s+(?:вечной\s+мерзлот|мерзлот|ледник)|отступлени\w*\s+ледник'
    r'|сокращени\w*\s+(?:площади\s+льда|ледяного\s+покрова)', re.I)
_NATURAL_SYSTEM_RE = re.compile(
    r'мор\w{1,3}\b|озер\w*|залив\w*|ледник\w*|мерзлот\w*|почв\w*|риф\w*|корал\w*|лес\w*'
    r'|экосистем\w*|популяц\w*|водоём\w*|бассейн\w*|дельт\w*|тундр\w*|степ\w*|земл\w*'
    r'|вид\w*\s+животн|вид\w*\s+птиц|грунт\w*|побережь\w*', re.I)
_TECH_DEGRADATION_RE = re.compile(
    r'связност|сет\w*\s+связи|интернет|канал\w*\s+связи|сервис\w*|платформ\w*', re.I)
# Самодостаточные признаки: сам термин уже называет природный процесс,
# второе условие для них избыточно.
_IRREVERSIBLE_SOLO_RE = re.compile(
    r'опустынивани\w*|обесцвечивани\w*\s+корал|вымирани\w*\s+вид'
    r'|таяни\w*\s+вечной\s+мерзлот|исчезновени\w*\s+ледник', re.I)
# Текущее действие в заголовке: глагол настоящего или прошедшего времени,
# описывающий происходящее сейчас. Отменяет права ретроспективного потолка.
_PRESENT_ACTION_RE = re.compile(
    r'\b(отмен\w*|закрыв\w*|эвакуир\w*|останавлив\w*|приостанов\w*|вводит|ввел\w*|'
    r'объявил\w*|сообщил\w*|начал\w*|продолжа\w*|усилива\w*|готовит\w*|'
    r'предупрежда\w*|ограничив\w*|перекрыв\w*)', re.I)

# Ретроспективный заголовок: событийное слово вместе с прошедшим годом.
# Требуется год НЕ текущий и не будущий, иначе «наводнения 2026 года»
# попадут в тот же класс.
_RETRO_TITLE_RE = re.compile(
    r'(наводнен\w*|пожар\w*|землетрясен\w*|извержен\w*|ураган\w*|цунами|'
    r'катастроф\w*|авари\w*|теракт\w*|крушени\w*|обрушени\w*|атак\w*|удар\w*|'
    r'похорон\w*|останк\w*|поминов\w*)'
    r'[^.]{0,30}\b(19\d{2}|20[0-1]\d|202[0-5])\s*год'
    r'|\b(19\d{2}|20[0-1]\d|202[0-5])\s*год\w*[^.]{0,30}'
    r'(наводнен\w*|пожар\w*|землетрясен\w*|извержен\w*|ураган\w*|цунами|'
    r'катастроф\w*|авари\w*|теракт\w*|крушени\w*|обрушени\w*)', re.I)

# Ретроспективный ущерб: жертвы прошлых лет в аналитическом материале.
_RETRO_DAMAGE_RE = re.compile(
    # Корень «погиб», а не «погибл» (07.09.2026): форма «погибших» не
    # покрывалась, и «около 100 погибших в атаке 2023 года» проходило мимо.
    # Добавлены «убит» и «останки»: похороны и извлечение тел описываются
    # именно так.
    r'(?:унесл\w*|погиб\w*|жертв\w*|ранил\w*|разрушил\w*|убит\w*|останк\w*)'
    r'[^.]{0,80}?(?:19|20)\d{2}'
    r'|(?:19|20)\d{2}\s*год\w*[^.]{0,80}?(?:унесл|погиб|жертв|ранил|разрушил|убит)'
    # Событие, названное давностью словами: «три года назад», «полгода назад»
    r'|(?:\d+|дв[ае]|три|четыре|пять|шесть|семь|восемь|девять|десять|почти\s+\w+)'
    r'\s*(?:года|лет|месяц\w*)\s+назад', re.I)

_IRREVERSIBLE_FLOOR = 62
...
def estimate_severity(title, desc, bias=0, weight=1.0):
    ...
    high = ['war','killed','invasion','collapse','nuclear','explosion','coup',
            ...
            # ЭКОНОМИКА · СИСТЕМНЫЙ УРОВЕНЬ (01.09.2026). Аудит ленты: медиана
            # экономики 34 против 55 у геополитики. Причина не в мире, а в
            # словаре: у экономического события нет слов разрушения, и даже
            # дефолт оставался на базовом уровне. Здесь только то, что означает
            # разрыв, а не колебание.
            # Только системный разрыв, не единичный случай: банкротство одной
            # компании и остановка одного завода перенесены в med, иначе
            # закрытие магазина весило бы как дефолт страны.
            'дефолт','default','обвал','crash','коллапс',
            'девальвац','devaluation','гиперинфляц','hyperinflation','деноминац',
            'заморозка активов','заморозили активы','заморозка счетов',
            'отключение от swift','отключены от swift','эмбарго','embargo',
            'банковский кризис','набег на банки','bank run','неплатёжеспособн',
            'остановка биржи','торги приостановлен','дефицит ликвидности',
            'долговой кризис','debt crisis','реструктуризация долга',
            'кризис неплатежей','волна банкротств','массовые банкротства',
            'массовые увольнения','массовое сокращение']
    med = ['crisis','conflict','protest','sanctions','strike','flood','drought',
           ...
           # ЭКОНОМИКА · ДАВЛЕНИЕ. Не разрыв, но накопление: эти конструкции
           # описывают процесс, который в геополитике был бы назван нарастанием.
           'ключевая ставка','повышение ставки','ослабление рубля','ослаб рубль',
           'падение курса','рекордный минимум','исторический минимум','просрочен',
           'задолженност','неплатеж','отток капитала','отток вкладов','сокращение резервов',
           'снижение производства','падение выпуска','падение спроса','падение продаж',
           'дефицит бюджета','секвестр','экспортные ограничени','импортные ограничени',
           ...
           # Названный отраслевой кризис (04.09.2026): «бензиновый кризис»,
           # «топливный кризис» это состоявшееся состояние рынка, а не
           # прогноз. Слово «кризис» в med уже есть, но одиночное совпадение
           # не отражает отраслевой охват.
           # Только НАЗВАННЫЙ отраслевой кризис: «повышение тарифов» и
           # «вторичные эффекты» проверены и сняты как слишком общие,
           # они ловят обсуждение проезда и кризис доверия.
           # Долговые события (04.09.2026): «допустил полноценный дефолт при
           # выплате купона», «допущены техдефолты по двум выпускам» —
           # словарь видел здесь одно слово «дефолт» и не различал масштаб.
           'техдефолт','технический дефолт','дефолт при выплате','дефолт по купону',
           'урегулированию обязательств','урегулирования обязательств',
           'не исполнил обязательства','просрочка по облигац','купонный платёж',
           'бензиновый кризис','топливный кризис','энергетический кризис',
           'продовольственный кризис','платёжный кризис','кризис неплатежей',
           'тарифов жкх','инфляционная пружина',
           ...
           # Ценовой скачок и военная премия (04.09.2026). «WTI снова выше 91,
           # премия за войну возвращается, плюс 10,02% за неделю» не давало НИ
           # ОДНОГО совпадения: словарь не видел ни двузначного недельного
           # роста, ни самого понятия военной премии в цене.
           'премия за войну','военная премия','геополитическая премия',
           'самый высокий уровень с','самого высокого уровня с',
           'выше 90 долларов','выше 100 долларов','снова выше',
           # Ценовой рекорд без года отсчёта: «послевоенный рекорд», «рекорд
           # на фьючерсном рынке». Вчерашние формы требовали года и такие
           # заголовки пропускали (01.09.2026).
           # «послевоенн» отдельно не берём: «послевоенное восстановление
           # экономики» это рост, а не шок. Нужна связка с рекордом.
           'рекорда на','послевоенного рекорда','послевоенному максимуму',
           'послевоенный рекорд','рекордная цена','рекордные цены',
           'исторический рекорд','абсолютный рекорд','взлетают до','взлетели до',
           'резко выросли','резко подорожал','подскочил',
           'доходности облигаций','ралли нефти','цены на нефть выше',
           'нефть к 100','выше 100 долларов','ценовой шок','скачок цен']
    # ГРАНИЦА СЛОВА ДЛЯ КОРОТКИХ КОРНЕЙ (04.09.2026). Совпадение считалось
    # простым вхождением подстроки, и «удар» срабатывал внутри слова
    # «госУДАРственный»: заявление про Государственную думу получало сильный
    # маркер военного удара. На текущем срезе таких событий 14.
    # Для коротких корней из списка _HIGH_WORD_BOUNDARY требуется начало
    # слова; остальные проверяются как раньше, чтобы не менять поведение
    # длинных однозначных маркеров.
    kw_high = 0
    for s_kw in high:
        if s_kw in _HIGH_LEFT_BOUNDARY:
            if re.search(r'(?:^|[^а-яёa-z])' + re.escape(s_kw), text, re.I):
                if s_kw == 'обвал' and not re.search(
                        r'(?:^|[^а-яёa-z])обвал(?!ов[кч])', text, re.I):
                    pass
                else:
                    kw_high += 1
        elif s_kw in _HIGH_FULL_WORD:
            if re.search(r'(?:^|[^а-яёa-z0-9])' + re.escape(s_kw) +
                         r'(?:[аыиеойюя]{0,3})?(?![а-яёa-z])', text, re.I):
                kw_high += 1
        elif s_kw in text:
            kw_high += 1
    ...
    # Тот же приём, что с топливным порогом: поднимает до минимума, но
    # никогда не снижает. Класс узкий по построению — на корпусе из 752
    # событий признак срабатывает у одного, — и это ожидаемо: события,
    # где длительность названа вместе с величиной, редки, но каждое из
    # них описывает процесс, а не эпизод.
    if (_TREND_DURATION_RE.search(text) and _TREND_MAGNITUDE_RE.search(text)
            and _sev_out < _TREND_FLOOR):
        _sev_out = _TREND_FLOOR
    # Ретроспективный ущерб проверяется в редакционном слое (_editorial_pass),
    # а не здесь: estimate_severity считает только новые записи, а событие,
    # уже стоящее в ленте, держит оценку из кэша и сюда не попадает.
    # НЕОБРАТИМАЯ ДЕГРАДАЦИЯ ПРИРОДНОЙ СИСТЕМЫ (01.09.2026).
    # «Деградация Каспийского моря: уровень падает рекордно» получало 48,
    # «Таяние вечной мерзлоты: критическая угроза инфраструктуре» — 38.
    # Формула считает по маркерам ущерба, а у такого процесса нет ни жертв,
    # ни разрушений в моменте: он необратим в человеческом горизонте и не
    # имеет обратной связи, то есть тяжесть определяется не событием, а
...
_CANON_TYPE = [
    ...
    # ПРИЧИНА ПРИОРИТЕТНЕЕ ЭФФЕКТА (04.09.2026). Правило промышленной аварии
    # стоит выше военного и ловило «удар беспилотника по НПЗ, начался пожар»:
    # горящий завод это следствие удара, а не авария производства.
    (r'(?:беспилотник|бпла|дрон|ракетн\w*\s+удар|обстрел|авиауд|атак\w*\s+беспилотн)'
     r'[^.]{0,120}(?:завод|комбинат|нпз|нефтебаз|терминал|цех|предприят|пожар|возгоран)'
     r'|(?:завод|комбинат|нпз|нефтебаз|терминал|цех|предприят)[^.]{0,120}'
     r'(?:беспилотник|бпла|дрон|ракетн\w*\s+удар|обстрел|авиауд)', 'Военные удары'),
    ...
    # ПРИЧИНА ПРИОРИТЕТНЕЕ ЭФФЕКТА (04.09.2026). «В результате атаки
    # беспилотников на Сочи произошло возгорание на территории нефтебазы»
    # получало тип «Топливный рынок» и домен экономика: правила пожара и
    # топлива стоят выше военного и срабатывали на ПОСЛЕДСТВИИ удара.
    # Пожар и нефтебаза здесь следствие, а событие военное.
    # Признак узкий: пожар или топливный объект вместе с явным военным
    # маркером в одном тексте.
    (r'(?:пожар|возгоран|очаг|нефтебаз|топлив|азс)[^.]{0,120}'
     r'(?:беспилотник|бпла|дрон|ракетн\w*\s+удар|обстрел|авиауд|атак\w*\s+беспилотн)'
     r'|(?:беспилотник|бпла|дрон|ракетн\w*\s+удар|обстрел|авиауд)[^.]{0,120}'
     r'(?:пожар|возгоран|очаг|нефтебаз|топлив|азс)', 'Военные удары'),
...
_ECON_BATCH3_INSERT = [
    ...
    (r'пузыр\w*\s+(?:на\s+рынке|актив|недвижимост|крипт|ии|ai)|перегрет\w*\s+рынок|перегрев\s+рынка|коррекци\w*\s+(?:рынка|цен\s+на\s+недвижимост)|обвал\s+(?:цен\s+на\s+недвижимост|крипт|биткоин)|ипотечн\w*\s+кризис|переоценённ\w*\s+актив|спекулятивн\w*\s+(?:рост|пузыр)|переоценк\w*\s+(?:актив|дол\w*|акци|портфел|стоимост)|бумажн\w*\s+(?:прибыл|доход)|доход\w*\s+(?:лишь\s+|только\s+)?на\s+бумаге', 'Пузырь активов'),
]
...
def _editorial_gate(events):
    ...
    for e in events:
        ...
        # РЕТРОСПЕКТИВНЫЙ УЩЕРБ (02.09.2026). Прежнее правило ловило только
        # прямые отсылки к дате: годовщина, исполняется N лет, N лет назад в
        # этот день. Материал, который просто опирается на прошлую катастрофу,
        # под него не подпадал: «Ранее предупреждения о вулканах могут
        # означать больше ложных тревог» держало 90/100, потому что в теле
        ...
        # ДВУЗНАЧНЫЙ ЦЕНОВОЙ СКАЧОК (04.09.2026). Ставится здесь, а не в
        # estimate_severity: там результат затирается пересчётом по масштабу.
        if (_PRICE_JUMP_RE.search(t) and int(e.get('severity') or 0) < _PRICE_JUMP_FLOOR):
            e['severity'] = _sev_log(e, 'price_jump_floor', e.get('severity'),
                                     _PRICE_JUMP_FLOOR,
                                     'двузначный ценовой скачок за короткое окно', 'boost')
        ...
        # Уточнение 07.09.2026: условие «в заголовке нет событийной лексики»
        # пропускало ретроспективу, названную своим именем. «Смертоносные
        # наводнения 2021 года: исследователи выявляют пробелы» держало
        # 90/100, потому что слово «наводнения» в заголовке принималось за
        # признак текущего события. Заголовок с событийным словом И прошедшим
        # годом рядом — это отсылка к прошлой катастрофе, а не сообщение о ней.
        # Дополнительное условие: заголовок не должен описывать текущее
        # действие. «Индонезия отменяет рейсы из-за извержения» имеет ретро
        # в теле (цунами 2018 года), но само событие сегодняшнее, и потолок
        # к нему применяться не должен.
        elif (_RETRO_DAMAGE_RE.search(t)
                and (not _SIC_EVENT.search(str(e.get('title') or '').lower())
                     or _RETRO_TITLE_RE.search(str(e.get('title') or '')))
                and not _PRESENT_ACTION_RE.search(str(e.get('title') or ''))
                and int(e.get('severity') or 0) > 55):
            retro += 1
            e['severity'] = _sev_log(e, 'retro_damage_cap', e.get('severity'), 55,
                                     'ущерб прошлых лет в аналитическом материале', 'penalty')
        kept.append(e)
...
def _tg_classify(text):
    ...
    for _n in ('дискредит',):
        t = t.replace(_n, '§')
    # «кредит доверия» — про политический рейтинг, а не про заём (30.08.2026).
    # «Макрон утратил кредит доверия сограждан» получало домен Экономика.
    # Тот же класс, что «дискредитация»: слово настоящее, смысл переносный.
    for _n in ('кредит доверия', 'кредита доверия', 'кредиту доверия'):
        t = t.replace(_n, '§')
...
# ИЗМЕРЕННЫЙ ЭКСТРЕМУМ ПОКАЗАТЕЛЯ (01.09.2026). Вынесен отдельно от общего
# словаря событий: только к нему применяется фильтр прогноза. «Аналитики ждут
# рекорда» событием не является, а «удар вызвал пожар, прогноз ухудшен» —
# является, и общий словарь не должен страдать от слова «прогноз» в тексте.
_SIC_EXTREMUM = _re_sic.compile(
    # «установить в рекордных количествах» — оборот речи, а не измерение:
    # глагол «установить» из списка снят, «рекордные количества» требуют
    # существительного рекорд/максимум, а не прилагательного.
    r'(?:достиг\w*|обнови\w*|взлет\w*|подня\w*|упал\w*|опусти\w*|'
    r'превыси\w*|побил\w*)\s+(?:\w+\s+){0,4}(?:рекорд[ауове]?\b|максимум\w*|минимум\w*|антирекорд\w*)'
    r'|(?:рекордн\w*|максимальн\w*|минимальн\w*)\w*\s+(?:уровн|значени|отметк|показател)\w*\s+с\s+\d{4}'
    r'|на\s+(?:максимуме|минимуме)\s+с\s+\d{4}'
    # Впервые пересечённый порог: «рождаемость впервые опустилась ниже уровня
    # воспроизводства». Это измерение показателя, а не мнение о нём.
    r'|впервые[^.]{0,45}(?:опустил\w*|подня\w*|превыси\w*|упал\w*|достиг\w*|пересек\w*)'
    r'|(?:опустил\w*|упал\w*|подня\w*|превыси\w*)[^.]{0,30}впервые', _re_sic.I)

_SIC_FORECAST = _re_sic.compile(
    r'ожида\w*|прогноз\w*|мож\w*\s+(?:достичь|подняться|вырасти|упасть)|'
    r'вероятно|по мнению|аналитик\w*\s+(?:счита|ожида|прогноз)|риск\w*\s+(?:роста|падения)',
    _re_sic.I)
...
_SIC_EVENT = _re_sic.compile(r'('
  ...
  r'дефолт|обвал|крах|заморож\w+ актив|'
  # СОСТОЯВШЕЕСЯ ФИНАНСОВОЕ ДЕЙСТВИЕ (01.09.2026). Словарь событийной лексики
  # построен на катастрофах и кинетике: у размещения облигаций, закрытия сделки
  # или выхода на биржу нет ни одного признака происшествия, и такие записи
  # уходили в контекст. «Саудовская Аравия продала облигации на 3,25 млрд
  # долларов во вторник» — свершившееся наблюдаемое действие с суммой и датой,
  # это событие, а не комментарий.
  # Требуется глагол в совершённой форме: намерение и планы остаются контекстом.
  r'(?:продал|размести\w*л|привлек|привлёк|выкупи\w*л|разместил)\w*\s+(?:\w+\s+){0,3}'
  r'(?:облигац|бумаг|долг|акци|евробонд)|'
  r'закрыл\w*\s+сделку|завершил\w*\s+размещение|провёл размещение|'
  r'выш\w*л\w*\s+на\s+биржу|состоялось размещение|'
  # ИЗМЕРЕННЫЙ ЭКСТРЕМУМ ПОКАЗАТЕЛЯ (01.09.2026). «Цены на дизель взлетают до
  # нового послевоенного рекорда» уходило в контекст: словарь событийной
  # лексики не содержит ни одной формы про выход показателя за исторический
  # диапазон. Между тем цена наблюдается непрерывно, и достижение рекорда —
  # это измерение, а не мнение. Тот же статус, что у маловодья и блэкаута:
  # мониторинг-феномен является событием по природе (ADR-003).
  # Требуется СОСТОЯВШИЙСЯ экстремум: прогнозы и ожидания остаются контекстом.
  r'состоялось размещение|'
  # КИБЕР-НАБЛЮДЕНИЯ (01.09.2026). Сплошная проверка: из 35 событий домена
  # технологий 15 стояли в контексте, включая три уязвимости ServiceNow с
  # баллом 10.0. Словарь содержит только физические утечки и слово «взлом»;
  # раскрытие уязвимости, публикация эксплойта и выкладывание украденных
  # данных под него не подпадали, хотя это состоявшиеся факты.
  r'уязвимост\w*[^.]{0,40}(?:получил|обнаружен|раскрыт|опубликован|балл|cvss|критическ)|'
  r'(?:получил\w*|присвоен\w*)[^.]{0,20}(?:cvss|балл\s*10|максимальн\w*\s+балл)|'
  r'эксплойт\w*[^.]{0,30}(?:обходит|опубликован|выпущен|появил)|'
  r'выложил\w*\s+данные|опубликовал\w*\s+(?:украденн|похищенн)\w*\s+данные|'
  r'слил\w*\s+данные|утечк\w*\s+данных|скомпрометирован\w*)', _re_sic.I)
...
def _sic_class(title, summary='', canon_type=None):
    ...
    def _base():
        ...
        # Экстремум в прогнозе не является наблюдением (01.09.2026):
        # «аналитики ожидают нового рекорда», «цены могут достичь максимума».
        # Событийный признак снимается, только если он единственный: при
        # наличии свершившегося действия прогноз в том же тексте не мешает.
        if not has_event and _SIC_EXTREMUM.search(low) and not _SIC_FORECAST.search(low):
            has_event = True
...
def save_enriched(events, previous_snapshot=None):
    ...
    if _SIGNAL_ENRICHER_AVAILABLE:
        try:
            ...
            # БАЗОВЫЙ УРОВЕНЬ БЕЗ НАБЛЮДЕНИЯ (01.09.2026).
            # Аудит показал: 78 событий ленты из 380 стоят ровно на 32 — это
            # база плюс вес источника, то есть формула не нашла ни одного
            # маркера. Число не оценка, а отметка «оценить не удалось».
            # Если при этом в тексте нет и признаков наблюдаемого события,
            # запись уходит в контекстный слой: она остаётся доступной, но
            # не занимает место в оперативной ленте.
            # Порог 34 включительно: 32 и 34 — два соседних значения базового
            # уровня, различие между ними даёт только вес источника.
            _OBS_MARK = re.compile(
                r'\d+\s*(?:погиб|жертв|пострадав|раненых|человек|тыс|млн|млрд|га\b|%)'
                r'|наводнен|пожар\w|засух|ураган|тайфун|шторм|землетряс|эвакуац|разруш'
                r'|остановлен|обесточ|повреждён|повреждено|введён\w* режим'
                r'|объявлен\w* чрезвычайн|атак\w|удар\w* по|обстрел|взлом|утечк|сбой'
                r'|дефицит|отключ', re.I)
            for _bl in enriched["events"]:
                if (_bl.get('severity') or 0) <= 34 and _bl.get('feed_visible') is not False:
                    if not _OBS_MARK.search((_bl.get('title') or '') + ' ' + (_bl.get('summary') or '')):
                        _bl['feed_visible'] = False

            # ТЕЛЕВИЗИОННЫЙ ДАЙДЖЕСТ (04.09.2026). «Главные новости в утреннем
            # выпуске: 0:00-0:39 столкновения в Чили, 0:42-1:22 атаки на
            # российские регионы, 1:23-3:05 санкции Евросоюза…» — это
            # оглавление эфира, а не событие. У него нет ни собственного
            # места, ни собственной оценки: он наследует их у первого
            # упомянутого сюжета, отсюда «Чили, Экономика, 44».
            # Признак: таймкоды в тексте либо прямое указание на выпуск.
            # Имя отличается от существующего _DIGEST_RE (новостные бюллетени,
            # строка 4131): затенение сломало бы то правило.
            _TV_DIGEST_RE = re.compile(
                r'\b\d{1,2}:\d{2}\s*[-–—]\s*\d{1,2}:\d{2}\b'
                r'|главные\s+новости[^.]{0,40}(?:выпуск|эфир)'
                r'|в\s+(?:утреннем|вечернем|дневном)\s+выпуске'
                r'|смотрите\s+в\s+эфире|подробнее\s+в\s+выпуске', re.I)
            for _dg in enriched["events"]:
                if _dg.get('feed_visible') is False:
                    continue
                _dt = (_dg.get('title') or '') + ' ' + (_dg.get('summary') or '')
                if _TV_DIGEST_RE.search(_dt):
                    _dg['feed_visible'] = False
                    _dg['noise_class'] = 'tv_digest'

            # СВЁРТКА ОДНОТИПНЫХ ПРЕДУПРЕЖДЕНИЙ (01.09.2026).
            # Официальные предупреждения Росгидромета — ценный ранний сигнал и
            # из ленты не убираются. Но однотипные по типу явления идут пачкой
            # по регионам: пять гроз по республикам Кавказа подряд, четыре
            # пожарные опасности. В ленте это выглядит как сбой, а не как
            # наблюдение. Первые три остаются, остальные уходят в контекст:
            # само предупреждение сохраняется, повтор типа не занимает экран.
            _CAP_LIMIT = 3
            _cap_seen = {}
            for _cw in sorted(enriched["events"],
                              key=lambda _x: -(_x.get('severity') or 0)):
                _src = str(_cw.get('source') or '')
                if 'Росгидромет' not in _src and 'CAP' not in _src:
                    continue
                _kind = str(_cw.get('title') or '').split(':')[0].strip().lower()
                if not _kind:
                    continue
                _cap_seen[_kind] = _cap_seen.get(_kind, 0) + 1
                if _cap_seen[_kind] > _CAP_LIMIT:
                    _cw['feed_visible'] = False
            ...
            # Гуманитарный отклик на катастрофу — не катастрофа (31.08.2026).
            # «Бар в Сентрале переполнен: жители Гонконга собирают 50 000
            # гонконгских долларов пострадавшим от наводнения в Непале»
            # получало severity 87 и место Непал: оценка и география взяты у
            # упомянутой катастрофы, хотя наблюдаемое — сбор средств в другой
            # стране. Прежний шаблон ловил только прямое обращение
            # («сделайте пожертвование»), а не описание сбора.
            _SH_DONATE=re.compile(r'пожертвовани\w+|будет удвоен|как вы можете (?:спасти|помочь)|поддержите нас|ваш\w+ (?:взнос|донат)|donate|fundrais'
                r'|сбор\w*\s+средств'
                r'|собра\w*[^.!?]{0,70}?(?:для\s+помощи|в\s+пользу\s+пострадавш|пострадавш\w+|'
                r'жертв\w*\s+(?:наводнен|землетрясен|катастроф|пожар|урагана))'
                r'|благотворительн\w*\s+(?:вечер|акци|аукцион|марафон|концерт)'
                r'|charity\s+(?:event|night|drive)', re.I)
