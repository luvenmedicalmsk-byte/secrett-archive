...
def fetch_copernicus():
    ...
    # RSS активаций CEMS
    feeds = [
        "https://emergency.copernicus.eu/mapping/activations/rss",
        "https://emergency.copernicus.eu/mapping/list-of-activations-rapid/rss",
    ]
    for url in feeds:
        data = fetch_url(url)
        if not data: continue
...
def fetch_nasa_firms(api_key=None):
    ...
    key = api_key or os.environ.get('FIRMS_API_KEY', 'DEMO_KEY')
...
def fetch_flood_observatory():
    ...
    feeds = [
        "https://floodobservatory.colorado.edu/rss/flood_events.rss",
