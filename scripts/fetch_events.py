...
# Триллионы: «Госдолг США превысил 40 триллионов» разбиралось как число
# 40 без единицы измерения, масштаб терялся полностью.
_NUM_RE = re.compile(r'(\d[\d\u00a0\u202f ]*\d|\d)(?:[.,](\d+))?\s*(трлн|триллион\w*|млрд|миллиард|млн|миллион|тыс\.?|тысяч[аи]?)?(?![\d])', re.I)
...
def _numbers(text):
    ...
    for m in _NUM_RE.finditer(text):
        ...
        mu=(m.group(3) or '')
        if mu.startswith(('трлн','триллион')): n*=1e12
        elif mu.startswith(('млрд','миллиард')): n*=1e9
