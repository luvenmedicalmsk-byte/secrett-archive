...
def _is_news_not_signal(title, summary, domain):
    ...
    _fluff = any(w in b for w in (
        ...
        'отвечает на ваши вопросы','размышления о','размышлен','рассужден','колонка:','колумнист','авторская колонка','профессор кафедры','мнение:',' эссе','почему я ',' weekend','уикенд','деньги в эфире','наш max'))
    _local = (
...
_DOMAIN_VOCAB = {
    ...
    'technology': ('кибератак','хакер','взлом','утечк','уязвим','cve','ddos','вредоносн','malware','дата-центр','цод','облач','спутник','глонасс','gps','нейросет','искусственн интеллект','gmail','телеком','оператор связи','отключ интернет','энергосет','подстанц','платёжн','авторизац','сервер','радиац','аэс','реактор'),
    'social': ('эпидеми','пандеми','вспышк','заболевани','вирус','инфекц','воз ','больниц','здравоохран','врач','миграц','беженц','демограф','рождаемост','смертност','убыль','голод','продовольств','карантин','вакцин','образован','школ','студент','женщин','девочк','гуманитарн','недоедан','пенси','пенсион','соцфонд','социальн фонд','пособи','прожиточн','инвалид'),
...
def _risk_gate(events):
    ...
    sys_p = ('Ты — фильтр платформы мониторинга СИСТЕМНЫХ РИСКОВ. Для каждого элемента входного '
             ...
             'а также новости об ОТДЕЛЬНОЙ компании или бренде без системных последствий (корпоративная рутина, закупки техники и оборудования, запуск или обзор продукта, проблемы одного магазина/ритейлера/сети, мелкие регуляторные курьёзы). '
