...
# ═══════════════════ PHASE 4: DOMAIN EXPLAINABILITY (PEX-16..20) ═══════════════════
def _domain_of(sig):
    return (sig.get('primary_domain') or (sig.get('domains') or [''])[0] or '').lower()

# Этап 2: «почему это один процесс» — доменная лексика
_DOMAIN_WHY_ONE = {
    'climate': 'Все события отражают развитие одного климатического процесса, проявляющегося в нескольких взаимосвязанных регионах.',
    'economy': 'Все события относятся к одной экономической динамике, проявляющейся в разных секторах и регионах.',
    'geopolitics': 'Все события являются проявлениями одного геополитического процесса, развивающегося на взаимосвязанных территориях.',
    'social': 'Все события отражают развитие одного социального процесса, влияющего на связанные группы населения и регионы.',
    'technology': 'Все события относятся к развитию одного технологического процесса, проявляющегося в нескольких связанных инцидентах или изменениях.',
}

# Этап 3: «что означает состояние» — доменная интерпретация при активном/растущем процессе
_DOMAIN_STATE_ACTIVE = {
    'climate': 'Процесс продолжает распространяться на новые территории, что говорит о сохранении климатической активности.',
    'economy': 'Процесс продолжает влиять на экономическую систему и расширяет область своего воздействия.',
    'geopolitics': 'Процесс остаётся активным и сопровождается новыми взаимосвязанными событиями.',
    'social': 'Процесс продолжает развиваться и затрагивает всё больше социальных проявлений.',
    'technology': 'Процесс остаётся активным и сопровождается новыми технологическими событиями или инцидентами.',
}

# Этап 4: «почему важно наблюдать» — аналитическая значимость (не прогноз, PEX-19)
_DOMAIN_WHY_WATCH = {
    'climate': 'Расширение географии процесса свидетельствует о сохранении климатической активности и требует дальнейшего наблюдения.',
    'economy': 'Распространение процесса на несколько регионов указывает на формирование устойчивой экономической динамики.',
    'geopolitics': 'Продолжение взаимосвязанных событий подтверждает сохранение напряжённости в регионе.',
    'social': 'Расширение числа проявлений говорит о закреплении социальной тенденции.',
    'technology': 'Новые подтверждения свидетельствуют о продолжающемся развитии технологического процесса.',
}

def why_watch(sig):
    """Этап 4: почему важно наблюдать (аналитическая значимость, не прогноз PEX-19).
    Показываем только для активных/растущих — у затухающих особой срочности нет."""
    stage = sig.get('lifecycle_stage') or ''
    if stage in ('Завершён', 'Ослабление'):
        return None
    dom = _domain_of(sig)
    return _DOMAIN_WHY_WATCH.get(dom)

def why_one_process(sig):
    """Почему Atlas считает это одним процессом — доменным языком (PEX-16)."""
    dom = _domain_of(sig)
    dt = _DOMAIN_WHY_ONE.get(dom)
    if dt:
        return dt
    # fallback для доменов вне списка
