...
async function handleAuthTelegram(request, env){
  ...
  if (!_activeNow(c)) {
    let _kvb = !!env.CLIENTS_KV, _raw = null;
    try { if (env.CLIENTS_KV) _raw = await env.CLIENTS_KV.get('client:'+tgId); } catch(_e) { _raw = 'ERR:'+_e.message; }
    return jsonResponse({ error:'ID '+tgId+' · CLIENTS_KV:'+(_kvb?'привязан':'НЕ привязан')+' · запись:'+(_raw===null?'НЕТ':String(_raw).slice(0,50)), auth:'no_access', tg_id: tgId, kv_bound:_kvb }, 403);
  }
