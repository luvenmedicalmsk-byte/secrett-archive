...
async function _classifyNewsItems(items, env){
  ...
  for (const it of items){ it._rkey = 'risk3:' + (it.url || ('x/' + it.id)); }
  ...
  if (todo.length && env.OPENAI_API_KEY){
    const sys = 'Ты — фильтр ленты платформы мониторинга СИСТЕМНЫХ РИСКОВ. Для каждого элемента реши: это СИГНАЛ системного риска или ШУМ, и определи домен. СИГНАЛ: война, удары, обстрелы, ракеты, вооружённые конфликты, санкции, протесты, перевороты, теракты, стихийные бедствия, аварии инфраструктуры, кибератаки, утечки данных, обвалы рынков, дефолты государств, банковские и секторальные кризисы, резкие движения валют и цен, эпидемии, гуманитарные кризисы, крупные политические и правовые события с последствиями. ШУМ: реклама, промо, самопиар канала, подкасты, культура и кино, лайфстайл, знаменитости, спорт, гороскопы, опросы, рецепты, анонсы мероприятий, А ТАКЖЕ новости об ОТДЕЛЬНОЙ компании или бренде без системных последствий (банкротство или проблемы одного ритейлера, магазина, сети; запуск продукта; корпоративная рутина). ДОМЕН (важно): если есть военные действия, удары, ракеты, война, вооружённый конфликт — домен geopolitics, ДАЖЕ если упомянута экономическая инфраструктура (НПЗ, порты, нефть, заводы). economy — только системные и макро-экономические события. ШКАЛА s (0-100), главный фактор — человеческие жертвы и масштаб: 90-100 — массовые жертвы, ядерная или радиационная угроза, эскалация войны, катастрофа с многими погибшими; 75-89 — удары, теракты, атаки С ПОГИБШИМИ, крупные стихийные бедствия с жертвами; 60-74 — события с ранеными или пострадавшими без погибших, серьёзные аварии без жертв; 45-59 — материальные и стратегические события БЕЗ жертв (захват судна, санкции, пожар на объекте без пострадавших, единичный обстрел без потерь, перехват дронов без последствий); 35-44 — напряжённость, переговоры, дипломатия, угрозы и заявления без действий. При прочих равных событие С человеческими жертвами получает индекс ВЫШЕ, чем событие того же типа без жертв. Верни СТРОГО валидный JSON-объект: ключ = значение поля i (строкой), значение = объект {\"r\":1 если сигнал риска иначе 0,\"d\":домен из [climate,economy,geopolitics,technology,social],\"s\":индекс риска целое 0-100 (0 для шума)}. Без markdown и пояснений.';
