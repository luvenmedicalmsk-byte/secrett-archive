...
// === S42/S44/S45 для TG-ленты: домен/сигнал-шум/severity по содержанию (на переведённом русском) ===
const _TG_SIG_RE = /войн|военн|армия|войск|ракет|дрон|бпла|обстрел|санкц|эмбарго|пошлин|тариф|нато|оон|переговор|саммит|дипломат|посол|кремл|пентагон|переворот|протест|митинг|забастовк|мобилизац|конфликт|границ|боев|теракт|взрыв|стрельб|захват|вторжен|наступлен|оккупац|аннекс|референдум|импичмент|sanction|war|military|missile|airstrike|coup|инфляц|безработиц|ввп|gdp|рецесс|дефолт|default|банкрот|bankrupt|ставк|обвал|crash|кризис|crisis|нефт|газ|баррел|бюджет|дефицит|госдолг|ипотек|экспорт|импорт|производств|увольнен|сокращен|подорожан|рубл|доллар|евро|юан|валют|бирж|котировк|акци|облигац|инвест|энерг|recession|эпидеми|пандеми|вспышк|outbreak|заболевани|вирус|инфекц|больниц|здравоохран|миграц|беженц|демограф|смертност|голод|продовольств|отравлен|карантин|вакцин|кибератак|хакер|взлом|утечк|уязвим|cve|ddos|вредоносн|malware|дата-центр|цод|облачн|спутник|глонасс|gps|нейросет|блэкаут|энергосет|подстанц|телеком|платёжн|погиб|жертв|killed|dead|пострадал|эвакуац|чрезвычайн|режим чс|разрушен|обрушен|катастроф|disaster|наводнен|землетряс|ураган|цунами|радиац|nuclear|пожар|шторм/i;
const _TG_DOMVOCAB = {
  geopolitics: ['войн','военн','армия','армии','войск','ракет','дрон','бпла','беспилотник','сбил','сбит','авиаудар','удар','обстрел','санкц','эмбарго','пошлин','нато','оон','переговор','саммит','дипломат','посол','мид','госдеп','кремл','пентагон','переворот','мобилизац','конфликт','границ','боев','теракт','оккупац','аннекс','вторжен','наступлен','путин','трамп','зеленск','киев','еврокоми','танкер','судоходств','всу','пво','снаряд','госпереворот','украин','евротрой','еврочетвёрк','ядерн'],
  economy: ['инфляц','безработиц','ввп','рецесс','дефолт','банкрот','ставк','бирж','обвал','нефт','баррел','бюджет','дефицит','госдолг','ипотек','экспорт','импорт','производств','завод','увольнен','сокращен','подорожан','рубл','доллар','евро','юан','валют','облигац','инвест','жил','недвижим','логистик','поставк','тариф','азс','бензин','топлив','росстат','спрос'],
  technology: ['кибератак','хакер','взлом','утечк','уязвим','cve','cisa','ddos','вредоносн','malware','дата-центр','цод','облач','спутник','глонасс','gps','нейросет','gmail','телеком','оператор связи','энергосет','подстанц','платёжн','авторизац','сервер','радиац','аэс','реактор'],
  social: ['эпидеми','пандеми','вспышк','заболевани','вирус','инфекц','воз ','больниц','здравоохран','врач','миграц','беженц','демограф','рождаемост','смертност','убыль','голод','продовольств','карантин','вакцин','образован','школ','студент','женщин','девочк','гуманитарн','недоедан','пенси','пенсион','соцфонд','пособи','прожиточн','инвалид'],
  climate: ['наводнен','паводок','землетряс','оползен','шторм','ураган','тайфун','циклон','засух','лесн пожар','ливень','снегопад','аномальн жар','вулкан','цунами','сель ','штормов','пожар','очаг','задымл',' дым','смог','режим чс','эвакуир']
};
function _tgReclass(text, cur){
  const b = (text||'').toLowerCase();
  let best = cur, bestN = -1, curN = 0;
  for (const d in _TG_DOMVOCAB){ const n = _TG_DOMVOCAB[d].reduce((a,w)=>a+(b.indexOf(w)>=0?1:0),0); if (d===cur) curN=n; if (n>bestN){ bestN=n; best=d; } }
  return (best!==cur && bestN>=1 && bestN>curN) ? best : cur;
}
function _tgIsNews(text, domain){
...
// === Этап 8: уровень подтверждённости геосигналов (детерминированно, на переведённом тексте) ===
var _CFM_EXPECT = ['ожида','планир','может привести','не исключен','рассматрива','предполага','прогнозир'];
var _CFM_NEGOT = ['переговор','обсужда','ведут диалог','консультаци по','раунд перегов','готовят соглаш','на стадии согласов'];
var _CFM_PRELIM = ['предварительн','проект соглаш','меморандум','рамочн','договорённост о намерени','договоренност о намерени','согласова рамк','близки к соглаш','приблизились к соглаш'];
var _CFM_STATE = ['призва','пригроз','предлож','предупрежда','предостерег','выразил готовность','выступил с инициатив','пообещ','заявил о намерени','заявил о готовн','сообщил о намерени','грозит'];
var _CFM_DEALVERB = ['достигл','подписа','объяв','согласова','договорил','заключ'];
var _CFM_DEALNOUN = ['соглашени','договорённост','договоренност','перемири','о мире','мирное соглаш','мирн договор','мирн план','мирн урегулир','прекращени войн','прекращени конфликт','прекращени огн'];
function _cfmHas(b, arr){ for (var i=0;i<arr.length;i++){ if (b.indexOf(arr[i])>=0) return true; } return false; }
function _cfmGrand(b){
  if (_cfmHas(b,_CFM_DEALVERB) && _cfmHas(b,_CFM_DEALNOUN)) return true;
  if (b.indexOf('пролив')>=0 && (b.indexOf('открыт')>=0 || b.indexOf('возобновл')>=0)) return true;
  if (b.indexOf('ормуз')>=0 && (b.indexOf('открыт')>=0 || b.indexOf('возобновл')>=0)) return true;
  return false;
}
function _tgConfirmLevel(text){
  var b = (text||'').toLowerCase();
  if (_cfmHas(b,_CFM_EXPECT)) return 'expectation';
  if (_cfmHas(b,_CFM_STATE)) return 'statement';
  if (_cfmHas(b,_CFM_NEGOT)) return 'negotiation';
  if (_cfmHas(b,_CFM_PRELIM)) return 'preliminary';
  if (_cfmGrand(b)) return 'preliminary';
  return 'confirmed';
}
var _CFM_CAP = { expectation:44, statement:46, negotiation:50, preliminary:55 };
var _CFM_LABEL = { expectation:'Ожидается', statement:'Заявление', negotiation:'Переговоры', preliminary:'Предварительно' };
var _CFM_SOFTEN = [['достигли соглашения','сообщили о предварительной договорённости'],['Достигли соглашения','Сообщили о предварительной договорённости'],['достигли договорённости','сообщили о предварительной договорённости'],['Достигли договорённости','Сообщили о предварительной договорённости'],['объявляют о соглашении','сообщили о предварительной договорённости'],['Объявляют о соглашении','Сообщили о предварительной договорённости'],['объявили о соглашении','сообщили о предварительной договорённости'],['Объявили о соглашении','Сообщили о предварительной договорённости'],['согласовали прекращение','сообщили о предварительной договорённости о прекращении'],['Согласовали прекращение','Сообщили о предварительной договорённости о прекращении'],['договорились о','сообщили о предварительной договорённости о'],['Договорились о','Сообщили о предварительной договорённости о'],['подписали соглашение','договорились подписать соглашение'],['Подписали соглашение','Договорились подписать соглашение'],['подписано соглашение','согласован проект соглашения'],['Подписано соглашение','Согласован проект соглашения']];
...
function _tgSeverity(text, domain, sev){
  ...
  const has = (arr)=>arr.some(w=>b.indexOf(w)>=0);
  const routineWx = has(['гроза','дожд','ливень','ветер','шквал','туман','гололёд','гололед','снегопад','метел','жара','высокая температура','пожарная опасность','прочие опасности','заморозк','сильный снег','осадк']);
  const majorWx = has(['наводнен','паводок','шторм','ураган','тайфун','циклон','цунами','землетряс','оползен','прорыв','эвакуир','погиб','жертв','разрушен','катастроф']);
  if (domain==='climate' && routineWx && !majorWx) sev = Math.min(sev, 38);
  const disaster = has(['наводнен','паводок','циклон','тайфун','ураган','шторм','оползен','землетряс','лесной пожар','сель']);
  const natscale = has(['вся страна','по всей стране','национальн','столиц','чрезвычайн положени','несколько стран','десятки тысяч','сотни погиб','тысяч эвакуир','государственн бедств','штатов']);
  if (domain==='climate' && disaster && !natscale) sev = Math.min(sev, 60);
  if (has(['cve','cisa','уязвим','vulnerab',' kev']) && !has(['critical infrastructure','критическ инфра','массов','wormable','energy grid','энергосет','банковск','national','общенацио'])) sev = Math.min(sev, 52);
  if (has(['бытов','единичн','локальн пожар','местн житель'])) sev = Math.min(sev, 42);
  const deescal = has(['соглашен','сделк','перемири','переговор','урегулир','ceasefire','truce','peace deal','peace talks','negotiat']) || (b.indexOf('прекращени')>=0 && (b.indexOf('огн')>=0 || b.indexOf('войн')>=0)) || (b.indexOf('мирн')>=0 && has(['план','соглаш','договор','урегулир']));
  if (deescal) sev = Math.min(sev, 58);
  if (has(['глобальн','по всему миру','worldwide','несколько стран','страны ес','мировой рынок','пандеми','pandemic']) && has(['кризис','crisis','война','войн','war','санкц','sanction','дефицит','обвал','крах','эпидеми','дефолт','default','коллапс','collapse','рецесс','recession','блэкаут','blackout','катастроф','эвакуац'])) sev = Math.max(sev, 76);
  if (has(['газопровод','нефтепровод','санкц','sanction','ядерн','nuclear','аэс','пролив','strait','коридор','энергосист','power grid','блэкаут','blackout','цепочк поставок','supply chain','swift','дефолт','default'])) sev = Math.max(sev, 62);
  return Math.max(12, Math.min(100, Math.round(sev)));
}
...
function _newsDomain(text, source) {
  ...
  if (has(['flood','наводнен','storm','шторм','hurricane','ураган','typhoon','тайфун','wildfire','пожар','earthquake','землетряс','quake','drought','засух','heatwave','heat wave','жара','climate','климат','emission','выброс','volcan','вулкан','eruption','tsunami','цунами','landslide','оползень','дамб','паводок','подтопл','затоплен','ливень','селев'])) return 'climate';
  // военные действия/удары/дроны -> geopolitics РАНЬШЕ economy: удар по НПЗ/топливу = последствие конфликта, не экономика
  if (has(['бпла','беспилотн','дрон','drone','обстрел','авиауд','ракет','missile','airstrike','война','войн','боевы','артиллер','удар по','shelling','теракт','terror','вторжен','наступлен'])) return 'geopolitics';
...
function _tgParseChannel(html, handle, name) {
  ...
  for (let k = 1; k < parts.length; k++) {
    ...
    text = text.replace(/\s*Прямой\s+эфир\s*[.!…]?\s*$/, '').trim();  // самоподпись novosti_efir
...
async function handleProxyNewsFeed(env) {
  ...
  // дроп локальных аварий/ЧП без системного значения (зеркало S41 пайплайна), на переведённом тексте
  items = items.filter(it => {
    const b = (it.text || '').toLowerCase();
    const combat = /(сбит|сбил|зенит|ракет|обстрел|атаков|удар по|уничтож|боеприпас|диверс|теракт|снаряд|дрон|бпла|всу|пво)/.test(b);
    const accident = /(крушени|авиакатастроф|разбил)/.test(b) && /(самолет|самолёт|вертолет|вертолёт|параплан|парашют|дельтаплан|легкомоторн)/.test(b);
    const gas = b.includes('взрыв') && /(газа|бытов|в жилом|в квартир|в доме|котельн|газовый баллон)/.test(b);
    const local = (/акул/.test(b) && /(атак|укус|напал|пострадал|погиб)/.test(b) && !/(подлод|субмарин|лодк|флот|учени|тихоокеан)/.test(b)) || b.includes('в колодец') || b.includes('поскользн') || /(изнасилов|педофил|маньяк)/.test(b);
    const fluff = /(нба|nba|нхл|нфл|ufc|плей-офф|футбол|хоккей|баскетбол|теннис|сборная по|олимпиад|mrbeast|млн подписчиков|подписчиков на youtube|ютубер|тиктокер|подарки на день|подарки ко дню|ко дню отца|что подарить|размышления о|отвечает на ваши вопросы)/.test(b);
    return !(fluff || local || ((accident || gas) && !combat));
  });
  // S43b: дроп локального шума (криминал-курьёзы, рутинный пенсионный админ); смягчение опровержений слухов
  items.forEach(function(it){ var b=(it.text||'').toLowerCase(); if(b.indexOf('не соответствуют действительности')>=0 && b.indexOf('якобы')>=0 && typeof it.severity==='number') it.severity=Math.min(it.severity,32); });
  items = items.filter(function(it){ var b=(it.text||'').toLowerCase();
    if(b.indexOf('прописан')>=0 && (b.indexOf('квартир')>=0||b.indexOf('госуслуг')>=0)) return false;
    if(b.indexOf('пенси')>=0 && b.indexOf('автоматическ')>=0 && b.indexOf('назнача')>=0) return false;
    return true;
  });
  ...
  (function(){
    function _sigNorm(k){ return (k||'').toLowerCase().replace(/[^a-z\u0430-\u044f\u04510-9 ]/gi,' ').split(/\s+/).filter(function(w){return w.length>=4;}).map(function(w){return w.slice(0,6);}).sort().join(' '); }
    var seen={}, coll=[];
    var order=items.slice().sort(function(a,b){ return (b.severity||0)-(a.severity||0); });
    order.forEach(function(it){
      var sig=_sigNorm(it._sig);
      if(sig && seen[sig]!=null){
        var keep=coll[seen[sig]];
        keep.dup_count=(keep.dup_count||0)+1;
        keep.dup_sources=keep.dup_sources||[];
        if(it.source && it.source!==keep.source && keep.dup_sources.indexOf(it.source)<0) keep.dup_sources.push(it.source);
        return;
      }
      if(sig) seen[sig]=coll.length;
      coll.push(it);
    });
  ...
  // Этап 8: уровень подтверждённости -- снижаем риск, смягчаем заголовок, ставим метку (geopolitics/economy)
  items.forEach(function(it){
    if (['geopolitics','economy'].indexOf(it.domain) < 0) return;
    var lvl = _tgConfirmLevel(it.text||'');
    if (lvl === 'confirmed') return;
    if (_CFM_CAP[lvl] && typeof it.severity === 'number') it.severity = Math.min(it.severity, _CFM_CAP[lvl]);
    it.confirmation = _CFM_LABEL[lvl] || '';
...
function _buildSignalProLocal(p) {
  ...
  // лидер по данным (уровень)
  const byLevel=DOMS.map(d=>({d,s:levels[d]})).sort((a,b)=>b.s-a.s);
  const dataLead=byLevel[0]&&byLevel[0].s>0?byLevel[0].d:null;
  const dataLevel=dataLead?levels[dataLead]:0;
  // «сырой» стратегический лидер по недемпфированному весу (для индекса расхождения)
  const rawStrat=DOMS.map(d=>({d,v:levels[d]*baseW(d)})).sort((a,b)=>b.v-a.v);
  const rawStratLead=rawStrat[0]?rawStrat[0].d:null;
  // ── ОГРАНИЧИТЕЛЬ ДОМИНИРОВАНИЯ: затухание странового веса при разрыве уровней ──
  const GAP_LO=12, GAP_HI=30, DAMP_MIN=0.4;
  const dampF=(d)=>{ const gap=Math.max(0, dataLevel-levels[d]); if(gap<=GAP_LO) return 1.0; if(gap>=GAP_HI) return DAMP_MIN; return 1 - (gap-GAP_LO)/(GAP_HI-GAP_LO)*(1-DAMP_MIN); };
  const dampedW=(d)=>{ const b=baseW(d); return 1 + (b-1)*dampF(d); };
  // ── факторы Strategic Impact Score (0..1) ──
  function factors(d){
    const levelN=clamp01(levels[d]/100);
    const baseWN=clamp01((dampedW(d)-1)/1.0);
    ...
    // ascent: высокий уровень + приближение к порогу 60 → растёт быстрее
    const ascentN=clamp01((levels[d]-50)/10)*(0.5+0.5*(fcUp?1:(g7!=null&&g7>0?0.5:0)));
    return {levelN,baseWN,thresholdN,concN,cascadeN,velocityN,forecastN,crossN,ascentN};
  }
  const WF={levelN:0.22,baseWN:0.10,thresholdN:0.12,concN:0.10,cascadeN:0.16,velocityN:0.10,forecastN:0.04,crossN:0.04,ascentN:0.12};
  ...
  const FLAB={levelN:'высокий уровень домена',baseWN:'стратегическая значимость для страны',thresholdN:'близость к порогу высокого риска',concN:'высокая концентрация сигналов',cascadeN:'вклад в каскадные эффекты',velocityN:'рост давления за последние дни',forecastN:'прогнозная траектория роста',crossN:'междоменное распространение',ascentN:'приближение к порогу высокого риска'};
  const NOTE={levelN:'высокого уровня домена',baseWN:'значимости для системных процессов страны',thresholdN:'близости к порогу высокого риска',concN:'концентрации сигналов',cascadeN:'высокой каскадности',velocityN:'роста давления',forecastN:'прогнозной траектории роста',crossN:'междоменного распространения',ascentN:'приближения к порогу высокого риска'};
  ...
  const top1=ranking[0]||null, top2=ranking[1]||null;
  const stratLead=top1?top1.d:null; const stratObj=top1; const stratLevel=top1?top1.level:'';
  // ── индекс расхождения Data–Strategy ──
  const levelGap=(dataLead&&rawStratLead&&dataLead!==rawStratLead)?Math.abs(dataLevel-levels[rawStratLead]):0;
  const gapCat=levelGap<12?'низкое':(levelGap<=25?'умеренное':'высокое');
  // ── смешанное лидерство ──
  const MIXED_GAP=10;
  const mixed=!!(top1&&top2&&top1.d!==top2.d&&(top1.sis-top2.sis)<=MIXED_GAP);
  const dataScore=dataLead?Math.round(dataLevel):null;
  // партнёр по смешанному лидерству — другой из топ-2
  const mixPartner = mixed ? (top1.d===dataLead?top2.d:top1.d) : null;
  const mixedNote = mixed ? ('Система фиксирует смешанное лидерство. Наибольший объём сигналов сосредоточен в домене '+(DL[dataLead]||dataLead)+', однако существенное стратегическое влияние сохраняет '+(DL[mixPartner]||mixPartner)+'.') : '';
  // домен под наблюдением
  const watchCand=scored.filter(x=>x.d!==stratLead).map(x=>({d:x.d,level:x.level,sis:x.sis,m:x.f.thresholdN*0.4+x.f.velocityN*0.3+x.f.forecastN*0.15+x.f.ascentN*0.15})).sort((a,b)=>b.m-a.m||b.sis-a.sis);
  ...
  // ── баланс факторов давления ──
  const pressureBalance={
    actual: gri!=null?Math.round(gri):Math.round(dataLevel),
    strategic: top1?top1.sis:0,
  ...
  let stratNote='';
  if(mixed) stratNote=mixedNote;
  else if(stratLead&&dataLead&&stratLead!==dataLead) stratNote='Несмотря на то, что наибольший объём сигналов сейчас фиксируется в домене '+(DL[dataLead]||dataLead)+', наибольшее стратегическое влияние оказывает '+(DL[stratLead]||stratLead)+' из-за '+joinRu(topFactors(stratObj.f,3).map(x=>NOTE[x.k]))+'.';

  const qual=(x)=> x>=0.5?'высокое':(x>=0.25?'умеренное':(x>0?'низкое':'незначительное'));
  const whyDom = mixed ? mixPartner : stratLead;
  const whyObj = scored.find(x=>x.d===whyDom) || stratObj;
  const wsf = whyObj?whyObj.f:{};
  const stratFactors={ domain:whyDom, cascade:qual(wsf.cascadeN||0), velocity:qual(wsf.velocityN||0), forecast:(fcUp?'траектория роста':'стабильная траектория'), cross:qual(wsf.crossN||0), country_weight:Math.round(dampedW(whyDom)*100)/100, country_weight_base:baseW(whyDom) };

  let changed7d='';
  if(mixed){ changed7d+='Смешанное лидерство: наибольший объём сигналов — в домене '+(DL[dataLead]||dataLead)+(dataScore!=null&&dataScore>0?(' ('+dataScore+'/100)'):'')+', существенное стратегическое влияние сохраняет '+(DL[mixPartner]||mixPartner)+'. '; }
  else if(stratLead&&dataLead&&stratLead!==dataLead){ changed7d+='Наибольший объём сигналов сейчас фиксируется в домене '+(DL[dataLead]||dataLead)+(dataScore!=null&&dataScore>0?(' ('+dataScore+'/100)'):'')+', однако наибольшее стратегическое влияние оказывает '+(DL[stratLead]||stratLead)+' (уровень: '+stratLevel.toLowerCase()+'). '; }
  else if(stratLead||dataLead){ changed7d+='Ведущий домен — '+(DL[stratLead||dataLead]||(stratLead||dataLead))+(dataScore!=null&&dataScore>0?(' ('+dataScore+'/100)'):'')+'. '; }
  ...
  let keyLocal='';
  if(mixed){ keyLocal='Локальное давление формируется одновременно несколькими доменами ('+(DL[dataLead]||dataLead)+' и '+(DL[mixPartner]||mixPartner)+'); единый стратегический лидер не выделяется. '; }
  else { keyLocal='Ключевым стратегическим источником локального давления сейчас выступает '+(stratLead?(DL[stratLead]||stratLead):'совокупность факторов')+(stratLead?(' (стратегическое влияние: '+stratLevel.toLowerCase()+')'):'')+'. '; }
