...
async function _classifyNewsItems(items, env){
  ...
  const todo = items.filter(it => !it._llm).slice(0, 80);   // лимит на запрос, остальное -- к следующему обновлению
...
async function handleProxyNewsFeed(env) {
  ...
  return new Response(JSON.stringify({ channels: NEWS_TG_CHANNELS.map(c => c.name), llm_active: !!env.OPENAI_API_KEY, count: items.length, items }), {
